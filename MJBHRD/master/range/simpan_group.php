...
if ($jumlah>0)
{
	...
	if($typeform=="tambah"){
		...
		$idTabel = $row[0];
		$sqlQuery = "INSERT INTO MJ.MJ_M_LINK_GROUP (ID, COMPANY, PERSON_ID, PERIODE_GAJI, ID_GROUP, CREATED_BY, CREATED_DATE, ASSIGNMENT_ID) 
		VALUES ( $idTabel, $company, $nama, '$periode', $group, $user_id, SYSDATE, $assign_id)";
		//echo $sqlQuery;exit;
		...
		for ($x=0; $x<$countID; $x++){
			...
			$sqlQuery = "INSERT INTO MJ.MJ_M_LINK_GROUP_DETAIL (ID, ID_LINK_GROUP, ID_ELEMENT, VALUE, CREATED_BY, CREATED_DATE) 
			VALUES ( $idTabel_detail, $idTabel, $arrTransID[$x], $arrDefault[$x], $user_id, SYSDATE)";
			//echo $sqlQuery;exit;
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
		}
		...
		//update
		$sqlQuery = "UPDATE MJ.MJ_M_LINK_GROUP SET COMPANY=$company, PERSON_ID=$nama, PERIODE_GAJI='$periode', ID_GROUP=$group, LAST_UPDATED_BY=$user_id, LAST_UPDATED_DATE=SYSDATE, ASSIGNMENT_ID = $assign_id WHERE ID=$hdid";
		$result = oci_parse($con,$sqlQuery);
		oci_execute($result);
		...
		for ($x=0; $x<$countID; $x++){
			...
			$idTabel_detail = $row[0];
			$sqlQuery = "INSERT INTO MJ.MJ_M_LINK_GROUP_DETAIL (ID, ID_LINK_GROUP, ID_ELEMENT, VALUE, CREATED_BY, CREATED_DATE) 
			VALUES ( $idTabel_detail, $hdid, $arrTransID[$x], $arrDefault[$x], $user_id, SYSDATE)";
			//echo $sqlQuery;exit;
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
		}
