...
$result = oci_parse($con, "SELECT COUNT(-1) FROM MJ.MJ_M_PLANT_AREA_DRIVER 
...
if ($jumlah>0)
{
	...
	if($typeform=="tambah"){
		//insert
		$resultSeq = oci_parse($con,"SELECT MJ.MJ_M_PLANT_AREA_DRIVER_SEQ.nextval FROM dual");
		oci_execute($resultSeq);
		$row = oci_fetch_row($resultSeq);
		$IdIjin = $row[0];
		$sqlQuery = "INSERT INTO MJ.MJ_M_PLANT_AREA_DRIVER (ID, MJ_M_NOMINAL_RITASI_DRIVER_ID, PLANT_ID, EFFECTIVE_START_DATE, EFFECTIVE_END_DATE, CREATED_BY, CREATED_DATE) 
		VALUES ( $IdIjin, '$plantArea', '$plant', TO_DATE('$tgl_awal', 'YYYY-MM-DD'), TO_DATE('$tgl_akhir', 'YYYY-MM-DD'), '$user_id', SYSDATE)";
		//echo $sqlQuery;
		...
		//update
		$result = oci_parse($con,"UPDATE MJ.MJ_M_PLANT_AREA_DRIVER SET MJ_M_NOMINAL_RITASI_DRIVER_ID='$plantArea', PLANT_ID='$plant', EFFECTIVE_START_DATE=TO_DATE('$tgl_awal', 'YYYY-MM-DD'), EFFECTIVE_END_DATE=TO_DATE('$tgl_akhir', 'YYYY-MM-DD'), LAST_UPDATED_BY='$user_id', LAST_UPDATED_DATE=SYSDATE WHERE ID='$hdid'");
		oci_execute($result); 
