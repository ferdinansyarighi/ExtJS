...
if($typeform=="tambah"){
	...
	if ($jumlah>0)
	{
		...
		$IdUser = $row[0];
		$sqlQuery = "INSERT INTO MJ.MJ_M_JAMLEMBUR (ID, APP_ID, PLANT_ID, POSITION_ID, JUMLAH_SHIFT, JUMLAH_LEMBUR, OVER_JAM_LEMBUR, STATUS, CREATED_BY, CREATED_DATE) VALUES ( $IdUser, " . APPCODE . ", $plant, $posisi_id, $jumShift, $jumLembur, '$overJam', '$status', $emp_id, SYSDATE)";
		//echo $sqlQuery;
	...
	$row = oci_fetch_row($resultSeq);
	$IdUser = $row[0]; */
	$result = oci_parse($con,"UPDATE MJ.MJ_M_JAMLEMBUR SET JUMLAH_SHIFT=$jumShift, JUMLAH_LEMBUR=$jumLembur, OVER_JAM_LEMBUR='$overJam', STATUS='$status', LAST_UPDATED_BY=$emp_id, LAST_UPDATED_DATE=SYSDATE WHERE ID='$hdid'");
	oci_execute($result); 
