...
FROM MJ.MJ_M_NOMINAL_RITASI 
WHERE LOCATION_GROUP='$location'
...
if ($jumlah>0)
{
	$data = "Location Group tersebut sudah pernah diinput.";
} else {
	if($typeform=="tambah"){
		//insert
		$resultSeq = oci_parse($con,"SELECT MJ.MJ_M_NOMINAL_RITASI_SEQ.nextval FROM dual");
		oci_execute($resultSeq);
		$row = oci_fetch_row($resultSeq);
		$IdIjin = $row[0];
		$sqlQuery = "INSERT INTO MJ.MJ_M_NOMINAL_RITASI (ID, LOCATION_GROUP, NOMINAL, EFFECTIVE_START_DATE, EFFECTIVE_END_DATE, CREATED_BY, CREATED_DATE) 
		VALUES ( $IdIjin, '$location', '$nominal', TO_DATE('$tgl_awal', 'YYYY-MM-DD'), TO_DATE('$tgl_akhir', 'YYYY-MM-DD'), '$user_id', SYSDATE)";
		//echo $sqlQuery;
		...
		//update
		$result = oci_parse($con,"UPDATE MJ.MJ_M_NOMINAL_RITASI SET LOCATION_GROUP='$location', NOMINAL='$nominal', EFFECTIVE_START_DATE=TO_DATE('$tgl_awal', 'YYYY-MM-DD'), EFFECTIVE_END_DATE=TO_DATE('$tgl_akhir', 'YYYY-MM-DD'), LAST_UPDATED_BY='$user_id', LAST_UPDATED_DATE=SYSDATE WHERE ID='$hdid'");
		oci_execute($result); 
