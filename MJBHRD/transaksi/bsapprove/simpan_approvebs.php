...
if(isset($_SESSION[APP]['user_id']))
{
	...
	for ($x=0; $x<$countID; $x++) 
	{
		...
		if ($typeform=='Setuju') 
		{
			...
			/* Modify by ISA 23-Jan-2019 karena yang bisa di approve status 0 dari user
			if($tingkat == 1){
			*/
			...
			*/
			...
			/*
			$queryCountAppr = oci_parse( $con, "
				SELECT  COUNT( * )
				FROM    MJ.MJ_T_APPROVAL
				WHERE   TRANSAKSI_ID = $TransID
				AND     TRANSAKSI_KODE = 'BON'
				AND     TINGKAT = $tingkat
				AND     SYSDATE BETWEEN CREATED_DATE AND ( CREATED_DATE + INTERVAL '1' MINUTE )
			" );
			oci_execute( $queryCountAppr );
			$rowCountAppr = oci_fetch_row( $queryCountAppr );
			$countAppr = $rowCountAppr[0];
			...
			if ( $countAppr != 0 ) {
				...
				VALUES ($hdid, " . APPCODE . ", '$emp_id', $TransID, 'BON', $tingkat, 'Approved', '$AlasanApp', '$emp_name', SYSDATE)";
				//echo $query;
			...
			*/
			...
			$resultNo = oci_parse($con,"
					SELECT DISTINCT BS.NO_BS, BS.TIPE, PJ.NAME DEPT_PEM, PP.NAME POS_PEM, PG.NAME
						...
						, 'Rp '||TRIM( TO_CHAR( BS.NOMINAL, '999,999,999.99' ) ) NOMINAL
						, BS.KETERANGAN, PPF.EMAIL_ADDRESS, PPF2.EMAIL_ADDRESS
...
";
					...
					$resultEmail = oci_parse($con,"SELECT EMAIL_ADDRESS, TRIM( INITCAP(TITLE) || FIRST_NAME || ' ' || LAST_NAME )
					--INITCAP(TITLE)||REGEXP_SUBSTR(previous_last_name,'[^-]+', 2, 3)
...
";
		...
		{ //Disapproved
			...
			$tglJt = $rowNo[13];
			$emailPembuat = $rowNo[14];
			$tingkatTerakhir = $rowNo[15];
			
			
			//echo 'aa'.$tingkatTerakhir;exit;
			/* DIPAKAI */
