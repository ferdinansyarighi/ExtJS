...
//$hdid=$_GET['hd_id']; 
	$count=0;
	$result = oci_parse($con, "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='PINJAMAN'");
	oci_execute($result);
	while($row = oci_fetch_row($result))
	{
		...
		WHERE APP_ID=" . APPCODE . " AND id=$TransID AND TRANSAKSI_KODE = 'PINJAMAN'";
		//echo $queryAtt;exit;
