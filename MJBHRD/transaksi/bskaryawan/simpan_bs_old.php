 ...
 if(isset($_POST['nama_user']))
  {
	...
	if($typeform == "tambah"){
		...
		if($statusBS != 'Approved'){
			$sqlQuery = "UPDATE MJ.MJ_T_BS SET PERUSAHAAN_BS = $perusahaan_bs, PENANGGUNG_JAWAB=$penanggungJawab, TIPE='$tipe', NOMINAL=$nominal, tgl_jt = to_date('$tgl_jt','YYYY-MM-DD'), KETERANGAN='$ket', AKTIF='$status', LAST_UPDATED_BY=$emp_id, LAST_UPDATED_DATE=SYSDATE, SYARAT1 = '$syarat1', SYARAT2 = '$syarat2', SYARAT3 = '$syarat3', SYARAT4 = '$syarat4', SYARAT5 = '$syarat5' WHERE 1=1 AND ID=$hdid ";
			//echo $sqlQuery;exit;
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
