...
SELECT BS.ID, BS.NO_BS, BS.ASSIGNMENT_ID, PPF.FULL_NAME PEMOHON, BS.PENANGGUNG_JAWAB, PPF2.FULL_NAME PJ, BS.NOMINAL
		, BS.KETERANGAN, BS.AKTIF, BS.TIPE, BS.PERUSAHAAN_BS, TO_CHAR(BS.TGL_JT, 'YYYY-MM-DD'), BS.SYARAT1, BS.SYARAT2, BS.SYARAT3, BS.SYARAT4, BS.SYARAT5
...
WHERE 1=1 AND ((BS.TINGKAT = 0 AND BS.STATUS NOT IN('Approved')) or (BS.TINGKAT = 2 AND BS.STATUS IN('Approved') AND BS.ASSIGNMENT_ID = BS.PENANGGUNG_JAWAB))
AND BS.ASSIGNMENT_ID = (SELECT distinct ASSIGNMENT_ID FROM PER_ASSIGNMENTS_F WHERE PERSON_ID = $emp_id AND EFFECTIVE_END_DATE > SYSDATE AND PRIMARY_FLAG='Y') $queryfilter
ORDER BY BS.ID
";
//echo $query;
$result = oci_parse($con,$query);
...
while($row = oci_fetch_row($result))
{
	...
	$record['DATA_PERUSAHAAN_BS']=$row[10];
	$record['DATA_TGL_JT']=$row[11];
	$record['DATA_SYARAT1']=$row[12];
	$record['DATA_SYARAT2']=$row[13];
	$record['DATA_SYARAT3']=$row[14];
	$record['DATA_SYARAT4']=$row[15];
	$record['DATA_SYARAT5']=$row[16];
	
	$record['TIPE_PENCAIRAN']=$row[17];
	$record['NO_REK']=$row[18];
	
	//$record['DATA_TIPE']=$row[10];
	$data[]=$record;
	$count++;
}
if ($count==0)
