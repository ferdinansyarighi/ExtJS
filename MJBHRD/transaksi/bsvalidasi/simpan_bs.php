 ...
 if(isset($_POST['hdid']))
  {
	...
	if($typeform == "edit"){
		...
		$query = "
			...
			LEFT JOIN PER_PEOPLE_F PPF ON PAF.PERSON_ID = PPF.PERSON_ID 
				AND PPF.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF.EFFECTIVE_END_DATE > SYSDATE
			LEFT JOIN PER_PEOPLE_F PPF2 ON BS.PENANGGUNG_JAWAB = PPF2.PERSON_ID 
		...
		if($countData == 0){
			...
			if ( $statusBS == 'Approved' ) {
				...
				$resultNo = oci_parse($con,"SELECT DISTINCT BS.NO_BS, BS.TIPE, PJ.NAME DEPT_PEM, PP.NAME POS_PEM, PG.NAME
						...
						, HOU.NAME, BS.TIPE_PENCAIRAN, DECODE(BS.TIPE_PENCAIRAN, 'TRANSFER', BS.NO_REK, ' - '), TO_CHAR(BS.TGL_PENCAIRAN, 'DD-Mon-YYYY')
						, HOU2.NAME, TO_CHAR(BS.TGL_JT, 'DD-MON-YYYY'), PPF3.EMAIL_ADDRESS
						FROM MJ.MJ_T_BS BS
