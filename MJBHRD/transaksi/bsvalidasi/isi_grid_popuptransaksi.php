...
$queryLogin = "SELECT COUNT(*) X FROM MJ.MJ_M_APPROVAL_PINJAMAN WHERE TIPE = 'BS' AND STATUS = 'A' AND KASIR_ID = $emp_id";
...
oci_execute($resultLogin);
$rowLogin = oci_fetch_row($resultLogin);
$kasirOrgId = $rowLogin[0]; 

if($countKasir == 0){
	$queryLogin = "SELECT CASE WHEN PJ.name like '%FIN%' THEN '5'
	...
	if ( $tingkat == 5 ) {
		...
		$query = "
			SELECT BS.ID, BS.NO_BS, BS.ASSIGNMENT_ID
				...
				, BS.PENANGGUNG_JAWAB, PPF2.FULL_NAME PJ, BS.NOMINAL
				...
				, BS.TIPE_PENCAIRAN, BS.NO_REK
				, TO_CHAR(BS.TGL_PENCAIRAN, 'YYYY-MM-DD'), REK_BANK_TRANSFER, NAME_BANK_TRANSFER, BANK_ACCOUNT_ID, BANK_NAME
				, NAMA_EXTERNAL, PENANGGUNGJAWAB_EXTERNAL, BS.STATUS
			...
			WHERE 1=1 AND BS.TINGKAT >= 5 
			AND BS.STATUS IN ( 'Approved', 'Validated' )
			-- and BS.TGL_PENCAIRAN IS NULL
			and BS.PERUSAHAAN_BS not in (SELECT PERUSAHAAN_ID FROM MJ.MJ_M_APPROVAL_PINJAMAN WHERE TIPE = 'BS' AND STATUS = 'A' AND KASIR_ID is not null)
			 $queryfilter
			ORDER BY BS.ID
			";
	}
	...
	$query = "
		SELECT BS.ID, BS.NO_BS, BS.ASSIGNMENT_ID
			...
			, BS.TIPE_PENCAIRAN, BS.NO_REK
			, TO_CHAR(BS.TGL_PENCAIRAN, 'YYYY-MM-DD'), REK_BANK_TRANSFER, NAME_BANK_TRANSFER, BANK_ACCOUNT_ID, BANK_NAME
			, BS.NAMA_EXTERNAL, BS.PENANGGUNGJAWAB_EXTERNAL, BS.STATUS
			, NVL( PPF.FULL_NAME, BS.NAMA_EXTERNAL ) PEMOHON_DECODE
			, NVL( PPF2.FULL_NAME, BS.PENANGGUNGJAWAB_EXTERNAL ) PENANGGUNGJAWAB_DECODE
		FROM MJ.MJ_T_BS BS
		LEFT JOIN APPS.PER_ASSIGNMENTS_F PAF ON BS.ASSIGNMENT_ID =PAF.ASSIGNMENT_ID 
		...
		-- and BS.TGL_PENCAIRAN IS NULL
		-- and (BS.PERUSAHAAN_BS = $kasirOrgId or
		AND (BS.PERUSAHAAN_BS in (SELECT PERUSAHAAN_ID FROM MJ.MJ_M_APPROVAL_PINJAMAN WHERE TIPE = 'BS' AND STATUS = 'A' AND KASIR_ID is not null AND KASIR_ID = $emp_id) or BS.PERUSAHAAN_BS not in (SELECT PERUSAHAAN_ID FROM MJ.MJ_M_APPROVAL_PINJAMAN WHERE TIPE = 'BS' AND STATUS = 'A' AND KASIR_ID is not null) )
		 $queryfilter
		ORDER BY BS.ID
		";
}
...
AND PPF2.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF2.EFFECTIVE_END_DATE > SYSDATE
WHERE 1=1 AND BS.TINGKAT = 5
--AND ((BS.ID IN (SELECT ID FROM MJ.MJ_T_BS WHERE NOMINAL <= 500000) AND BS.TINGKAT = 3) 
...
while($row = oci_fetch_row($result))
{
	...
	$record['TGL_PENCAIRAN']=$row[21];
	$record['REK_BANK_TRANSFER']=$row[22];
	$record['NAME_BANK_TRANSFER']=$row[23];
	$record['BANK_ACCOUNT_ID']=$row[24];
	$record['BANK_NAME']=$row[25];
	$record['NAMA_EXTERNAL']=$row[26];
	$record['PENANGGUNGJAWAB_EXTERNAL']=$row[27];
	$record['STATUS']=$row[28];	

	$record['PEMOHON_DECODE']=$row[29];
	$record['PENANGGUNGJAWAB_DECODE']=$row[30];
