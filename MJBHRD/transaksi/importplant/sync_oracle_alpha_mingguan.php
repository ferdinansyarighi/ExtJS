				...
				if($Kategori == 'Cuti'){
					...
					if($CutiTahunan == 0){
						...
						if ($CutiTahunan >= 1){
							...
							$query = "UPDATE MJ.MJ_M_CUTI SET CUTI_TAHUNAN = $CutiTahunan, LAST_UPDATED_BY='syncalphamingguan', LAST_UPDATED_DATE=SYSDATE WHERE PERSON_ID=$vPerson_id";
							$resultUpdate = oci_parse($con,$query);
							oci_execute($resultUpdate);
						}
					}
				} elseif ($Kategori == 'Sakit'){
					$queryCutiSakit = "SELECT (CUTI_SAKIT) AS  FROM MJ.MJ_M_CUTI WHERE PERSON_ID=$vPerson_id";
