					...
					foreach($data as $data_key => $data_value) {
						if ($countData > $columnStart){
							if($countData%2 == 0){
								...
								$queryCekBes = "SELECT TO_CHAR(TO_DATE('$newformat','YYYY-MM-DD'),'YYYY-MM-DD') FROM DUAL";
								$resultCekBes = oci_parse($con, $queryCekBes);
								oci_execute($resultCekBes);
								$rowCekBes = oci_fetch_row($resultCekBes);
								$CekBes=$rowCekBes[0];
								//echo $CekBes;exit;
								if($JamKeluar < $JamMasuk)
								{//echo 'a';exit;
									InsertTimeCard($con, $dataId, $newformat, $JamMasukAsli, $JamMasuk, $JamKeluarAsli, $JamKeluar, $user_id,3,$CekBes);
								}
								else {//echo $JamMasukAsli.$JamKeluarAsli;exit;
									InsertTimeCard($con, $dataId, $newformat, $JamMasukAsli, $JamMasuk, $JamKeluarAsli, $JamKeluar, $user_id,3,$newformat);
								}
							} else {
