<?PHP
 ...
 include 'D:/dataSource/MJBHRD/transaksi/importplant/fungsiTimecard.php';
...
$queryDate = "SELECT SYSDATE-14 FROM DUAL";
$resultDate = oci_parse($con,$queryDate);
oci_execute($resultDate);
$rowDate = oci_fetch_row($resultDate);
$vSdate = $rowDate[0];

	//ambil honors/id biofinger
	$queryHeader = "SELECT DISTINCT MK.KaryaCode, CONVERT(VARCHAR(10),DATEADD(day, -14, getdate()),120)
	FROM MastKarya MK WHERE MK.KaryaCode=600643 ";
	...
	while(odbc_fetch_row($resultHeader)){
		...
		if($CekGanti >= 1)
		{
			...
			FROM CardInOut CIO 
			WHERE CONVERT(VARCHAR(10), CIO.TranDate,120)=CONVERT(VARCHAR(10),DATEADD(day, -14, getdate()),120)
