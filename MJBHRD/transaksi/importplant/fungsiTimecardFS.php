...
function InsertTimeCard($con, $dataId, $newformat, $JamMasukAsli, $JamMasuk, $JamKeluarAsli, $JamKeluar, $user_id){
	...
	if($vID >= 1){
		$queryDataId = "SELECT COUNT(-1) FROM APPS.PER_PEOPLE_F WHERE PERSON_ID=$dataId AND EFFECTIVE_END_DATE > SYSDATE AND NVL(DATE_EMPLOYEE_DATA_VERIFIED, TO_DATE('12/31/4712', 'MM/DD/YYYY')) > SYSDATE";
		...
		if($countData >= 1){
			...
			//echo $HariIni . '<BR>' . $HariLibur . '<BR>' . $JamKeluarAsli . '<BR>' . $JamMasukAsli . '<BR>' . $CountMasuk;
						...
						if($CountKategori >= 1){
							...
							if($Kategori == 'Cuti'){
								...
								// if ($JamMasukAsli==''){
									...
									// $resultSeq = oci_parse($con,"SELECT MJ.MJ_T_TIMECARD_TEMP_SEQ.nextval FROM DUAL");
									// oci_execute($resultSeq);
									// $rowHSeq = oci_fetch_row($resultSeq);
									// $IdTimecard = $rowHSeq[0];
									
									// $queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$xJamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE)";
								...
								if($IjinKhusus == 'SETENGAH HARI'){
									...
									if ($JamMasukAsli==''){
										...
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$zJamMasukAsli', '$xJamKeluarAsli', '$zStatus', 'Belum', '$user_id', SYSDATE)";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
									}
						...
						if ($Masuk != '') {
							...
							if($JamKeluar > $Keluar){
								...
								if($tempJam >= 1){
									...
									$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamKeluarAsli', '$JamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE)";
									$result = oci_parse($con,$queryTimeCard);
									oci_execute($result);
								}
							}
							...
							$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$JamMasukAsli', '$JamKeluarAsli', '$Status', 'Belum', '$user_id', SYSDATE)";
							$result = oci_parse($con,$queryTimeCard);
							oci_execute($result);
						} else {
							if ($JamMasukAsli==''){
							...
							if($tempJam >= $cekLembur){
								...
								$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$JamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE)";
								$result = oci_parse($con,$queryTimeCard);
								oci_execute($result);
							} else {
								$resultSeq = oci_parse($con,"SELECT MJ.MJ_T_TIMECARD_TEMP_SEQ.nextval FROM DUAL");
					...
					if($CountMasuk >= 1){
						if ($Masuk != '') {
							if($JamMasuk >= $Terlambat){
								...
								$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_TEMP (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$JamMasukAsli', '$xStatus', 'Belum', '$user_id', SYSDATE)";
								$result = oci_parse($con,$queryTimeCard);
								oci_execute($result);
							} else {
								$Status=StatusTimecard($con, 'MASUK'); //Masuk
