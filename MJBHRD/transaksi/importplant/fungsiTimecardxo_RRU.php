...
/*
	echo "<br>InsertTimeCard<br>";
	echo "dataId=>".$dataId."<=newformat=>".$newformat."<=JamMasukAsli=>".$JamMasukAsli."<=JamMasuk=>".$JamMasuk."<=JamKeluarAsli=>".$JamKeluarAsli."<=JamKeluar=>".$JamKeluar."<=user_id=>".$user_id."<=pengurang=>".$pengurang."<=dateout=>".$dateout."<br>";
*/
		...
		if($countData >= 1){
			...
			if($countData >= 1){
				...
				if(($HariLibur == 1 && $CekOfff == 0) || ($CekOfff == 0 && $HariIni != 'SUNDAY') || ($HariIni == 'SUNDAY' && $CekOfff == 0)){
					...
					echo "<br>JamKeluarAsli=>".$JamKeluarAsli."<<=JamMasukAsli=>>".$JamMasukAsli."<br>";
					...
					if ($JamKeluarAsli=='' || $JamMasukAsli==''){
						...
						if($CountMasuk >= 1){
							...
							if($CountKategori >= 1){
								...
								if($Kategori == 'Cuti'){
									...
									// if ($JamMasukAsli==''){
										...
										// $resultSeq = oci_parse($con,"SELECT MJ.MJ_T_TIMECARD_XO_SEQ.nextval FROM DUAL");
										// oci_execute($resultSeq);
										// $rowHSeq = oci_fetch_row($resultSeq);
										// $IdTimecard = $rowHSeq[0];
										
										// $queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_XO (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$xJamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE)";
							...
							RRU*/			
										...
										echo "<br>JamMasukAsli=>".$JamMasukAsli."<JamKeluarAsli>".$JamKeluarAsli."<br>";
									}
								} else { 
									if($FlagATL == 'Y'){
...
/*								echo $JamMasuk.' >= '.$Terlambat;EXIT;
								if ($JamMasukAsli != ''){
									if($JamMasuk >= $Terlambat){
										...
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD_XO (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$oJamMasukAsli', '$JamMasukAsli', '$oStatus', 'Belum', '$user_id', SYSDATE)";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
									}
								}*/
					...
					RRU */	
						...
						echo "<br>CountMasuk=>".$CountMasuk."<=vJamMasuk=>".$vJamMasuk."<=vMenitMasuk=>".$vMenitMasuk."<=vJamKeluar=>".$vJamKeluar."<=vMenitKeluar=>".$vMenitKeluar."<br>";
						
						//echo $CountMasuk;exit;
						//echo $JamMasuk . ' dan ' . $Terlambat; 1434 dan 706
						if($CountMasuk >= 1){
							if ($Masuk != '') {
								echo "<br>JamMasuk=>".$JamMasuk."<=Terlambat=>".$Terlambat."<br>";
								//exit();
								if($JamMasuk >= $Terlambat){
									$xStatus=StatusTimecard($con, 'TERLAMBAT'); //Terlambat
									$Status=StatusTimecard($con, 'MASUK'); //Masuk
