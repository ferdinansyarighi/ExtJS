...
$vSdate = $rowDate[0];
		...
		WHERE MK.KaryaCode IN (600876,600643,600505,600519,600731,600759,600750,600779,600865,600888,600978,600981,600996,600993,60023,600709,600831,600373,600985,601017,600100,601004,601024,601032
		)";
		...
		while(odbc_fetch_row($resultHeader)){
			...
			if($CekGanti >= 1)
			{
				...
				if ($JamKeluarAsli == '' && $Masuk > $Keluar || $JamKeluar < $JamMasuk && $Masuk > $Keluar|| $JamMasukAsli == '' && $Masuk > $Keluar)
				{ 
			...
			InsertTimeCard($con, trim($dataId), $TglHeader, $JamMasukAsli, $JamMasuk, $JamKeluarAsli, $JamKeluar, 24);
					...
					if($cekShift >= 1)
					{
						...
						$JamKeluar = '';
							...
							if ($JamKeluarAsli == '' && $Masuk > $Keluar || $JamKeluar < $JamMasuk && $Masuk > $Keluar || $JamMasukAsli == '' && $Masuk > $Keluar || $Masuk > $Keluar)
							{
								if($JamKeluarBesok < $JamMasukBesok || $JamMasukBesok == '')
								{ 
									$JamKeluarAsli = substr($besok, 11, 5);
