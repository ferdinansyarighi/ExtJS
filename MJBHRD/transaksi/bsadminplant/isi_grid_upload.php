...
//$hdid=$_GET['hd_id']; 
//echo "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='BON'";exit;

	$count=0;
	$result = oci_parse($con, "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='BONADMIN' ");
	oci_execute($result);
	while($row = oci_fetch_row($result))
	{
		...
		WHERE APP_ID=" . APPCODE . " AND id=$row[0] AND TRANSAKSI_KODE = 'BONADMIN'";
		//echo $queryAtt;exit;
