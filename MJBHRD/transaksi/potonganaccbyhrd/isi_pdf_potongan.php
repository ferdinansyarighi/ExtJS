...
$queryHRD = "
	SELECT  DECODE( MTP.BYHRD, 'Y', 
			...
			, MTA.STATUS APV_HRD, MTA2.STATUS APV_DIR, MTA3.STATUS APV_MGR_FIN, MTA4.STATUS APV_MGR
			, PPF5.FULL_NAME MGR_NAME
			, TO_CHAR( MTA4.CREATED_DATE, 'DD-MON-YYYY HH24:MI:SS' ) TGL_APPR_MGR
			, MTP.STATUS_DOKUMEN, MTP.TINGKAT, MTA5.TINGKAT TGKT_APRV_LAST
	FROM MJ.MJ_T_PINJAMAN MTP
	...
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND TINGKAT = 2
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	) MTA ON MTP.ID = MTA.TRANSAKSI_ID
	...
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND TINGKAT = 3
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	) MTA2 ON MTP.ID = MTA2.TRANSAKSI_ID
	...
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND  TINGKAT = 4
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	) MTA3 ON MTP.ID = MTA3.TRANSAKSI_ID
	...
	LEFT JOIN (
        ...
        FROM (
			SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
			FROM MJ.MJ_T_APPROVAL
			WHERE TRANSAKSI_KODE = 'PINJAMAN' AND TINGKAT = 1
				AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
			ORDER BY ID DESC
        ) A
        UNION
        SELECT CREATED_DATE CREATED_DATE
        , 'Approved' STATUS, 'PINJAMAN' TRANSAKSI_KODE, 1 TINGKAT
        , MANAGER EMP_ID,ID TRANSAKSI_ID
        FROM MJ.MJ_T_PINJAMAN MTP
        WHERE ID = $hdid AND BYHRD = 'Y'
	) MTA4 ON MTP.ID = MTA4.TRANSAKSI_ID
	LEFT JOIN APPS.PER_PEOPLE_F PPF5 ON MTA4.EMP_ID = PPF5.PERSON_ID AND PPF5.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF5.EFFECTIVE_END_DATE > SYSDATE
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN'
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	) MTA5 ON MTP.ID = MTA5.TRANSAKSI_ID
...
$appr_hrd = $rowHRD[8];
$appr_dir = $rowHRD[9];
$appr_mgr_fin = $rowHRD[10];
$appr_mgr = $rowHRD[11];
$nama_mgr = $rowHRD[12];
$tgl_appr_mgr = $rowHRD[13];
$sts_doc = $rowHRD[14];
$tingkat = $rowHRD[15];
$tgkt_appr_last = $rowHRD[16];
	...
	*/
	...
	if ( $nama_mgr != ''){
		if (($appr_mgr == 'Disapproved') && ($sts_doc == 'Disapproved') && ($tgkt_appr_last >= 1))
			$pdf->Cell( 30, 25, $pdf->Image( 'disapprove-min.png', $pdf->GetX() + 0, $pdf->GetY() + 0, 32 ), 1, 0, 'L', false );
		else if (($appr_mgr == 'Approved') && (($tingkat >= 1) OR ($sts_doc != 'In process' && $tingkat < 1)))			
			$pdf->Cell( 30, 25, $pdf->Image( 'approve-min.png', $pdf->GetX() + 2, $pdf->GetY() + 5, 25 ), 1, 0, 'L', false );
		else
		{
			$tgl_appr_mgr = "";
	...
	if ( $nama_direksi != '' )  {
		if (($appr_dir == 'Disapproved') && ($sts_doc == 'Disapproved') && ($tgkt_appr_last >= 3))
			$pdf->Cell( 30, 25, $pdf->Image( 'disapprove-min.png', $pdf->GetX() + 0, $pdf->GetY() + 0, 32 ), 1, 0, 'L', false );
		else if (($appr_dir == 'Approved') && (($tingkat >= 3) OR ($sts_doc != 'In process' && $tingkat < 3)))
			$pdf->Cell( 30, 25, $pdf->Image( 'approve-min.png', $pdf->GetX() + 2, $pdf->GetY() + 5, 25 ), 1, 0, 'L', false );
		else
		{
			$tgl_appr_direksi = "";
	...
	if ( $nama_finance != '' ){
		if (($appr_mgr_fin == 'Disapproved') && ($sts_doc == 'Disapproved') && ($tgkt_appr_last >= 4))
			$pdf->Cell( 30, 25, $pdf->Image( 'disapprove-min.png', $pdf->GetX() + 0, $pdf->GetY() + 0, 32 ), 1, 0, 'L', false );
		else if (($appr_mgr_fin == 'Approved') && (($tingkat >= 4) OR ($sts_doc != 'In process' && $tingkat < 4))) 
			$pdf->Cell( 30, 25, $pdf->Image( 'approve-min.png', $pdf->GetX() + 2, $pdf->GetY() + 5, 25 ), 1, 0, 'L', false );
		else
		{
			$tgl_appr_finance = "";
	...
	if ($managerHRD != '' ){
		if (($appr_hrd == 'Disapproved') && ($sts_doc == 'Disapproved')&& ($tgkt_appr_last >= 2)) 
			$pdf->Cell( 30, 25, $pdf->Image( 'disapprove-min.png', $pdf->GetX() + 0, $pdf->GetY() + 0, 32 ), 1, 0, 'L', false );
		else if (($appr_hrd == 'Approved')  && (($tingkat >= 2) OR ($sts_doc != 'In process' && $tingkat < 2)))
			$pdf->Cell( 30, 25, $pdf->Image( 'approve-min.png', $pdf->GetX() + 2, $pdf->GetY() + 5, 25 ), 1, 0, 'L', false );
		else
		{
			$tglAppHRD = "";
	...
	/*
	if (($sts_doc == 'In process') && ($tingkat < 1))
		$tgl_app_mgr = "";
	if (($sts_doc == 'In process') && ($tingkat < 3))
		$tgl_appr_direksi = "";
	if (($sts_doc == 'In process') && ($tingkat < 4))
		$tgl_appr_finance = "";
	if (($sts_doc == 'In process') && ($tingkat < 2))
		$tglAppHRD = "";
	*/
	// $pdf->Cell( 30, 25, $pdf->Image( 'approve-min.png', $pdf->GetX() + 2, $pdf->GetY() + 5, 25 ), 1, 0, 'L', false );
	...
	$pdf->Cell(30, 8, 'Tgl :  ' . $tgl_appr_mgr, 'LR', 0, 'L', 0);
	...
	$pdf->Cell(30, 4, $nama_mgr, 'LRB', 0, 'C', 0);
