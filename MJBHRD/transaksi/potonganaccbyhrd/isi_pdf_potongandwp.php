...
--	SELECT MAX(ID) FROM MJ.MJ_T_APPROVAL WHERE TRANSAKSI_KODE = 'PINJAMAN' AND MTA.TINGKAT = 1 AND TRANSAKSI_ID = MTA.TRANSAKSI_ID
--)
LEFT JOIN (
...
$queryHRD = "
	...
	LEFT JOIN MJ.MJ_T_APPROVAL MTA ON MTP.ID = MTA.TRANSAKSI_ID AND MTA.TRANSAKSI_KODE = 'PINJAMAN' AND MTA.TINGKAT = 2 
	...
	LEFT JOIN MJ.MJ_T_APPROVAL MTA2 ON MTP.ID = MTA2.TRANSAKSI_ID AND MTA2.TRANSAKSI_KODE = 'PINJAMAN' AND MTA2.TINGKAT = 3
	...
	LEFT JOIN MJ.MJ_T_APPROVAL MTA3 ON MTP.ID = MTA3.TRANSAKSI_ID AND MTA3.TRANSAKSI_KODE = 'PINJAMAN' AND MTA3.TINGKAT = 4 
...
/* $queryHRD = "
	...
	SELECT  DECODE( MTP.BYHRD, 'Y', 
			...
			, PPF4.FULL_NAME FINANCE_NAME
			, TO_CHAR( MTA3.CREATED_DATE, 'DD-MON-YYYY HH24:MI:SS' ) TGL_APPR_FINANCE
			, MTA.STATUS APV_HRD, MTA2.STATUS APV_DIR, MTA3.STATUS APV_MGR_FIN
	FROM MJ.MJ_T_PINJAMAN MTP
	LEFT JOIN APPS.PER_PEOPLE_F PPF ON MTP.CREATED_BY = PPF.PERSON_ID AND PPF.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF.EFFECTIVE_END_DATE > SYSDATE
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND TINGKAT = 2 AND (STATUS = 'Approved' OR STATUS = 'Disapproved')
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	) MTA ON MTP.ID = MTA.TRANSAKSI_ID
	LEFT JOIN APPS.PER_PEOPLE_F PPF2 ON MTA.EMP_ID = PPF2.PERSON_ID AND PPF2.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF2.EFFECTIVE_END_DATE > SYSDATE
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND TINGKAT = 3 AND (STATUS = 'Approved' OR STATUS = 'Disapproved')
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
	...
	LEFT JOIN (
		SELECT CREATED_DATE, STATUS, TRANSAKSI_KODE, TINGKAT, EMP_ID, TRANSAKSI_ID
		FROM MJ.MJ_T_APPROVAL
		WHERE TRANSAKSI_KODE = 'PINJAMAN' AND  TINGKAT = 4 AND (STATUS = 'Approved' OR STATUS = 'Disapproved')
			AND ROWNUM <= 1 AND TRANSAKSI_ID = $hdid
		ORDER BY ID DESC
