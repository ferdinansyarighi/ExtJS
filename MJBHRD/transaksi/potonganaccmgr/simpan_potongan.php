 ...
 if(isset($_POST['hdid']))
  {
	...
	VALUES ( $IdTransAPP, " . APPCODE . ", '$emp_id', '$hdid', 'PINJAMAN', 1, '$typeform', '$keterangan', $emp_id, SYSDATE)";
	
	// VALUES ( $IdTransAPP, " . APPCODE . ", '$emp_id', '$hdid', 'PINJAMAN', 0, '$typeform', '$keterangan', $emp_id, SYSDATE)";
	
	$resultApp = oci_parse($con,$sqlQueryApp);
	oci_execute($resultApp);
	
	if($typeform == "Approved"){
		$sqlQueryA = "UPDATE MJ.MJ_T_PINJAMAN SET TINGKAT = 1, KETERANGAN_MGR = '$keterangan' WHERE ID = $hdid";
		$resultA = oci_parse($con,$sqlQueryA);
		oci_execute($resultA);
	} else {
		$sqlQueryA = "UPDATE MJ.MJ_T_PINJAMAN SET TINGKAT = 0, STATUS_DOKUMEN = '$typeform', KETERANGAN_MGR = '$keterangan' WHERE ID = $hdid";
		$resultA = oci_parse($con,$sqlQueryA);
		oci_execute($resultA);
	}
