...
while($row = oci_fetch_row($result))
{
	...
	$record['DATA_KETERANGAN']=$row[11];
	$record['DATA_TIPE']=$row[12];
	$record['DATA_MGR']=$row[13];
	$record['START_POTONGAN_BULAN']=$row[14];
	$record['START_POTONGAN_TAHUN']=$row[15];
	$record['START_POTONGAN']=$row[16];

	$record['DATA_TIPE_PENCAIRAN']=$row[17];
	$record['DATA_NOMOR_REKENING']=$row[18];
