...
if ($formtype=='tambah')
{
	...
	$querycekMano = "SELECT SUBSTR('$manager',0,15) FROM PER_PEOPLE_F";
	$resultcountmano = oci_parse($con,$querycekMano);
	oci_execute($resultcountmano);
	$rowcountmano = oci_fetch_row($resultcountmano);
	$jummano = $rowcountmano[0];
	
	// Mengambil Person ID Manager
	$querycekeMan = "SELECT PERSON_ID FROM PER_PEOPLE_F WHERE FULL_NAME LIKE '%$jummano%' AND FULL_NAME NOT LIKE '%TRIAL%' AND EFFECTIVE_START_DATE <= SYSDATE AND EFFECTIVE_END_DATE >= SYSDATE";
	...
	$querycountExe = "SELECT COUNT(-1) FROM MJ.MJ_T_RESIGN WHERE NAMA_KARYAWAN LIKE '%$nama_karyawan%' AND APPROVAL_MANAGER = 'Approved' AND APPROVAL_MANAGER_HRD = 'In Proccess' AND BY_HRD = 'Y' AND STATUS = 1";
	...
	$queryemailPem = "SELECT EMAIL_ADDRESS FROM PER_PEOPLE_F WHERE FULL_NAME LIKE '%$nama_karyawan%'";
