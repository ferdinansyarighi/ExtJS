...
for ($x=0; $x<$countID; $x++){
	...
	if($jumgen>=1){
		...
		if ($typeform=='Setuju'){
			$queryResign = "UPDATE MJ.MJ_T_RESIGN SET HRD_MGR = '$emp_name', APPROVAL_MANAGER_HRD='Approved', TGL_APP_TERAKHIR = SYSDATE, LAST_UPDATED_BY=$emp_id, LAST_UPDATED_DATE=SYSDATE 
			WHERE ID=$TransID";
