...
if ( $tipe == 'EXTERNAL' ) {
	...
	$nama_external=$_POST['nama_external'];
	$nama_external=str_replace("'", "`", $nama_external);
	
	$png_jwb_external=$_POST['png_jwb_external'];
	$png_jwb_external=str_replace("'", "`", $png_jwb_external);
	
	
	// $no_rek=str_replace( array( '"', "'" ), '', $no_rek );
	...
	*/
	...
	if($typeform == "tambah"){
		...
		if ($jumgen>0){
			...
			VALUES ($gencountseq, '$tahunGenNo', '0', '" . APPCODE . "', '', '', '', 'BON')";
		...
		// $noBS = "BS/EXTERNAL/" . $tahunGenNo. "/" . date('m')  . "/" . $nourut;
		
		$noBS = "BS/EXT/" . $tahunGenNo. "/" . date('m')  . "/" . $nourut;
		...
		$sqlQuery = "
		INSERT INTO MJ.MJ_T_BS 
			(	ID, NO_BS, 
				NOMINAL, KETERANGAN, TINGKAT, AKTIF, STATUS, 
				CREATED_BY, CREATED_DATE, TIPE, BYHRD, 
				PERUSAHAAN_BS, TGL_JT, SYARAT1, SYARAT2, SYARAT3, SYARAT4, SYARAT5,
				TIPE_PENCAIRAN, NO_REK, NAMA_EXTERNAL, PENANGGUNGJAWAB_EXTERNAL ) 
		VALUES ( 	$hdid, '$noBS', 
					$nominal, '$ket', 1, '$status', 
					'PROCESS', $emp_id, SYSDATE, '$tipe', 'Y', 
					$perusahaan_bs, to_date('$tgl_jt','YYYY-MM-DD'), '$syarat1', 
					'$syarat2', '$syarat3', '$syarat4', '$syarat5', 
					'$tipe_pencairan', '$no_rek', '$nama_external', '$png_jwb_external' )";
		
		
		// -- ASSIGNMENT_ID, PENANGGUNG_JAWAB, PERSON_ID, 
		// -- $assignment_id, $penanggungJawab, $person_id, 
		...
		// for ($x=0; $x<$countID; $x++){
			// $resultUpload = oci_parse($con,"UPDATE MJ.MJ_TEMP_UPLOAD SET TRANSAKSI_ID = $hdid WHERE TRANSAKSI_KODE = 'BONMGRHRD' AND FILENAME = '$arrNamaFile[$x]' AND TRANSAKSI_ID = 1");
			// oci_execute($resultUpload);
		// }
	...
	// Mode edit
		...
		if ( $statusBS != 'Approved' ) {
			...
			SET PERUSAHAAN_BS = $perusahaan_bs,
				...
				NO_REK = '$no_rek',
				NAMA_EXTERNAL = '$nama_external',
				PENANGGUNGJAWAB_EXTERNAL = '$png_jwb_external'
			WHERE 1=1
			AND ID=$hdid
			";
			...
			$queryUpload = "DELETE FROM MJ.MJ_M_UPLOAD WHERE APP_ID=" . APPCODE . " AND USERNAME='$user_id' AND TRANSAKSI_KODE='BONMGRHRD' AND TRANSAKSI_ID=$hdid";
			//echo $query;
			$resultUpload = oci_parse($con,$queryUpload);
			oci_execute($resultUpload);
	...
	$resultNo = oci_parse($con,"
        SELECT DISTINCT BS.NO_BS, BS.TIPE, PJ.NAME DEPT_PEM, PP.NAME POS_PEM, PG.NAME,
            ...
            PENANGGUNGJAWAB_EXTERNAL,
            'Rp ' || TRIM( TO_CHAR( BS.NOMINAL, '999,999,999.99' ) ) NOMINAL, BS.KETERANGAN
            , PPF.EMAIL_ADDRESS, PPF2.EMAIL_ADDRESS, 
            -- PPF.FULL_NAME, 
            NAMA_EXTERNAL,
            HOU.NAME, HOU2.NAME, TO_CHAR(BS.TGL_JT, 'DD-MON-YYYY'), PPF3.EMAIL_ADDRESS, BS.AKTIF
			FROM MJ.MJ_T_BS BS
	...
	if($data=="sukses" && $aktif == 'Y'){
		...
		//Cek apakah PIC accounting ada
		$resultCount = oci_parse($con,"
		SELECT COUNT(ACCOUNTING_ID) X 
		FROM MJ.MJ_M_APPROVAL_PINJAMAN 
		WHERE TIPE = 'BS' AND STATUS = 'A'
			AND PERUSAHAAN_ID = (
...
";
			...
			$resultEmail = oci_parse($con,"SELECT EMAIL_ADDRESS, INITCAP(TITLE)||FIRST_NAME||' '||LAST_NAME
...
} else 
{
	...
	if(isset($_POST['nama_user']))
	{
		...
		if($typeform == "tambah"){
			...
			$sqlQuery = "
			INSERT INTO MJ.MJ_T_BS 
				(	ID, NO_BS, ASSIGNMENT_ID, PENANGGUNG_JAWAB, NOMINAL, KETERANGAN, TINGKAT, AKTIF, STATUS, 
					CREATED_BY, CREATED_DATE, TIPE, BYHRD, PERSON_ID, PERUSAHAAN_BS, TGL_JT, SYARAT1, SYARAT2, SYARAT3, SYARAT4, SYARAT5,
							TIPE_PENCAIRAN, NO_REK, NAMA_EXTERNAL, PENANGGUNGJAWAB_EXTERNAL ) 
			VALUES ( $hdid, '$noBS', $assignment_id, $penanggungJawab, $nominal, '$ket', 1, '$status', 
					'PROCESS', $emp_id, SYSDATE, '$tipe', 'Y', $person_id, $perusahaan_bs, to_date('$tgl_jt','YYYY-MM-DD'), '$syarat1', 
					'$syarat2', '$syarat3', '$syarat4', '$syarat5', 
					'$tipe_pencairan', '$no_rek', '$nama_external', '$png_jwb_external' )";
		...
		$resultNo = oci_parse($con,"SELECT DISTINCT BS.NO_BS, BS.TIPE, PJ.NAME DEPT_PEM, PP.NAME POS_PEM, PG.NAME
				...
				, PPF.EMAIL_ADDRESS, PPF2.EMAIL_ADDRESS, PPF.FULL_NAME, HOU.NAME, HOU2.NAME, TO_CHAR(BS.TGL_JT, 'DD-MON-YYYY'), PPF3.EMAIL_ADDRESS, BS.AKTIF
				FROM MJ.MJ_T_BS BS
				INNER JOIN APPS.PER_ASSIGNMENTS_F PAF ON BS.ASSIGNMENT_ID =PAF.ASSIGNMENT_ID AND PAF.EFFECTIVE_END_DATE > SYSDATE AND PAF.PRIMARY_FLAG='Y'
				...
				INNER JOIN PER_PEOPLE_F PPF3 ON BS.CREATED_BY = PPF3.PERSON_ID AND PPF3.CURRENT_EMPLOYEE_FLAG = 'Y' AND PPF3.EFFECTIVE_END_DATE > SYSDATE
