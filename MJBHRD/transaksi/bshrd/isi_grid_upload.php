...
$org_name = "";
 if(isset($_SESSION[APP]['user_id']))
  ...
  {
	$id = $_GET['id_temp'];
	$querywhere .= " AND ID = $id";  
  }
//$hdid=$_GET['hd_id']; 
//echo "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='BON' $querywhere";exit;
	$count=0;
	$result = oci_parse($con, "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='BONMGRHRD' ");
	oci_execute($result);
	
	// echo "SELECT ID AS HD_ID , FILENAME AS DATA_FILE FROM MJ.MJ_TEMP_UPLOAD WHERE username='$user_id' AND TRANSAKSI_KODE='BONMGRHRD' "; exit;
	...
	while($row = oci_fetch_row($result))
	{
		...
		WHERE APP_ID=" . APPCODE . " AND id=$row[0] AND TRANSAKSI_KODE = 'BONMGRHRD'";
		//echo $queryAtt;exit;
