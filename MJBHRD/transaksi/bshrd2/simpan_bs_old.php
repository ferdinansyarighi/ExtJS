 ...
 if(isset($_POST['nama_user']))
  {
	...
	if($typeform == "tambah"){
		...
		VALUES ( $hdid, '$noBS', $assignment_id, $penanggungJawab, $nominal, '$ket', 1, '$status', 'PROCESS', $emp_id, SYSDATE, '$tipe', 'Y', $person_id, $perusahaan_bs, to_date('$tgl_jt','YYYY-MM-DD'), '$syarat1', '$syarat2', '$syarat3', '$syarat4', '$syarat5' )";
		//echo $sqlQuery;exit;
		...
		if($statusBS != 'Approved'){
			...
			TIPE='$tipe', NOMINAL=$nominal, tgl_jt = to_date('$tgl_jt','YYYY-MM-DD'), KETERANGAN='$ket', AKTIF='$status', STATUS='PROCESS' ,LAST_UPDATED_BY=$emp_id, LAST_UPDATED_DATE=SYSDATE, SYARAT1 = '$syarat1', SYARAT2 = '$syarat2', SYARAT3 = '$syarat3', SYARAT4 = '$syarat4', SYARAT5 = '$syarat5' WHERE 1=1 AND ID=$hdid ";
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
			
			//delete  file attachment PINJAMAN sebelum dilakukan insert sesuai data di temp upload
			
			$queryUpload = "DELETE FROM MJ.MJ_M_UPLOAD WHERE APP_ID=" . APPCODE . " AND USERNAME='$user_id' AND TRANSAKSI_KODE='BONMGRHRDHRD' AND TRANSAKSI_ID=$hdid";
			//echo $query;
			$resultUpload = oci_parse($con,$queryUpload);
			oci_execute($resultUpload);
			...
			FROM MJ.MJ_TEMP_UPLOAD
			WHERE APP_ID=" . APPCODE . " AND USERNAME='$user_id'
			AND TRANSAKSI_KODE='BONMGRHRDHRD'";
			$result = oci_parse($con, $query);
			oci_execute($result);
			while($row = oci_fetch_row($result))
			{
			...
			for ($x=0; $x<$countID; $x++){
				...
				if($countUpl == 0){
					...
					$queryUpload = "DELETE FROM MJ.MJ_TEMP_UPLOAD WHERE APP_ID=" . APPCODE . " AND USERNAME='$user_id' AND TRANSAKSI_KODE='BONMGRHRD' AND TRANSAKSI_ID=$hdid";
					//echo $query;
					$resultUpload = oci_parse($con,$queryUpload);
					oci_execute($resultUpload);
