...
$query = "
SELECT DISTINCT NVL( PPF.MIDDLE_NAMES, BS.NAMA_EXTERNAL ), BS.NO_BS
	 , NVL( PPF.FULL_NAME, BS.NAMA_EXTERNAL ) PEM
     , NVL( PPF2.FULL_NAME, BS.PENANGGUNGJAWAB_EXTERNAL )PJ
     , PJ.NAME DEPT_PEM, PP.NAME POS_PEM, PJ2.NAME DEPT_PJ, PP2.NAME POS_PJ
    ...
    , TO_CHAR(BS.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS') TGL_PENGAJUAN
    , NVL(to_char(MTA.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS')
    , TO_CHAR( MTA.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS' ) ) TGL_APP_MGR
    , MTA2.CREATED_BY APP_CKR
    , NVL( to_char( MTA2.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS')
    , TO_CHAR( MTA2.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS' ) ) TGL_APP_CKR
    , HOU2.NAME, TO_CHAR(BS.TGL_JT, 'DD - Mon - YYYY')
    , BS.SYARAT1, BS.SYARAT2, BS.SYARAT3, BS.SYARAT4, BS.SYARAT5
    , MTA3.CREATED_BY APP_HRD
    , to_char(MTA3.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS') TGL_APP_HRD
    , MTA4.CREATED_BY APP_CKR
    , to_char(MTA4.CREATED_DATE, 'DD-MON-YYYY HH:MI:SS') TGL_APP_CKR
	, BS.TIPE_PENCAIRAN || '   ' || BS.NO_REK TIPE_PENCAIRAN
    , BS.TIPE, BS.TINGKAT, BS.BYHRD, NVL(DECODE(BS.TIPE, 'SPPD', MTA2.KETERANGAN, MTA3.KETERANGAN),'    -') KET_CKR
            FROM MJ.MJ_T_BS BS
            LEFT JOIN APPS.PER_ASSIGNMENTS_F PAF ON BS.ASSIGNMENT_ID =PAF.ASSIGNMENT_ID AND PAF.EFFECTIVE_END_DATE > SYSDATE AND PAF.PRIMARY_FLAG='Y'
...
*/
	...
	/*
	$pdf->Cell(70, 6, 'ALASAN PERMOHONAN BON SEMENTARA', 'TBL', 0, 'R', 0);
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell(50, 6, '( di isi oleh Pemohon )', 'TBR', 0, 'L', 0);
	
	$pdf->Cell(60, 6, 'Bank Transfer', 'TBR', 0, 'C', 0);
	*/
	...
	// $pdf->MultiCell(120, 4, $keterangan, 1, 'L', 0);
	
	// $pdf->MultiCell(10, 4, '', 1, 'L', 0);
	...
	// $pdf->MultiCell(120, 4, 'Tipe Pencairan: ' . $tipe_pencairan, 1, 'L', 0);
	
	
	// $pdf->MultiCell(60, 4, '', 1, 'L', 0);
	
	// $pdf->Cell(60, 4, '', 1, 'L', 0);
	
	
	// $pdf->Cell(120, 4, 'Tipe Pencairan: ' . $tipe_pencairan, 1, 'L', 0);
	
	// $pdf->Cell(120, 4, 'Tipe Pencairan: ' . $tipe_pencairan, 'TBR', 0, 'C', 0);
	...
	// $pdf->MultiCell(60, 4, '', 1, 0, 'C', 0);
	
	
	$pdf->Cell(120, 4, 'Tipe Pencairan: ' . $tipe_pencairan, 'TBR', 'L', 0);
	
	$pdf->Cell(60, 4, 'Bank Transfer', 'TBR', 'C', 0);
	...
	*/
	...
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell(120, 4, '2. Penyelesaian Bon Sementara ini wajib disertai : ', 'LR', 0, 'L', 0);
	...
	$pdf->SetFont('Arial', '', 6);
	$pdf->Cell( 68.5, 4, '', 'R', 0, 'L', 0 );
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell( 51.5, 4, 'Bank Transfer', 'BR', 0, 'C', 0 );
	
	$pdf->Ln(4);
	
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);
	$pdf->Cell(60, 4, 'Tipe BS: ' . $tipe_bs, 'LR', 0, 'L', 0);
	
	$pdf->SetFont('Arial', '', 6);
	$pdf->Cell( 68.5, 4, 'BS PELUNASAN BULAN  ' . '_________________' . ' TAHUN  ' . '_______', 'LR', 0, 'L', 0 );
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell( 51.5, 4, '', 'LR', 0, 'L', 0 );
	...
	$pdf->SetFont('Arial', '', 6);
	$pdf->Cell( 68.5, 4, '', 'RB', 0, 'L', 0 );
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell( 51.5, 4, '', 'RB', 0, 'L', 0 );
	...
	$pdf->SetFont('Arial', '', 6);
	$pdf->Cell( 68.5, 10, 'BS PELUNASAN BULAN  ' . '_________________' . ' TAHUN  ' . '_______', 'RB', 0, 'L', 0 );
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell( 51.5, 10, '', 'RB', 0, 'L', 0 );
	...
	*/
	...
	if ( $by_hrd == 'Y' ) {
		...
		$pdf->Cell( 25.7, 4, 'Anggo, Mr. Freddy FRG', 'LRB', 0, 'C', 0);
	...
	*/
	...
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->SetFont('Arial', 'B', 7);
	$pdf->Cell( 180, 6, 'KETERANGAN CHECKER : ', 'LRT', 0, 'L', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->SetFont('Arial', '', 7);
	$pdf->MultiCell(180, 4, $keterangan_ckr, 'BLR', 'L', 0);
