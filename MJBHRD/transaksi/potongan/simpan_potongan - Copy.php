 ...
 if(isset($_POST['nama_user']))
  {
	...
	WHERE PPF.EFFECTIVE_END_DATE > SYSDATE AND PPF.FULL_NAME LIKE '$nama_user'";
	...
	WHERE APP_ID=" . APPCODE . " AND PERSON_ID=$person_id");
	...
	if ($userCount==0){
		...
		$sqlQuery = "INSERT INTO MJ.MJ_T_POTONGAN (ID, APP_ID, PERSON_ID, PINJAMAN_P, PINJAMAN_B, CICILAN_P, CICILAN_B, HARGA_CICILAN_P, HARGA_CICILAN_B, OUTSTANDING_P, OUTSTANDING_B, STATUS, CREATED_BY, CREATED_DATE) VALUES ( $IdUser, " . APPCODE . ", $person_id, $arrPinjamanP, $arrPinjamanB, $arrCicilanP, $arrCicilanB, $arrJumCicilanP, $arrJumCicilanB, $arrOutstandingP, $arrOutstandingB, 'A', $emp_id, SYSDATE)";
		$result = oci_parse($con,$sqlQuery);
		oci_execute($result);
			
		$data="sukses";
	} else {
		$sqlQuery = "UPDATE MJ.MJ_T_POTONGAN SET PINJAMAN_P=$arrPinjamanP, PINJAMAN_B=$arrPinjamanB, CICILAN_P=$arrCicilanP, CICILAN_B=$arrCicilanB, HARGA_CICILAN_P=$arrJumCicilanP, HARGA_CICILAN_B=$arrJumCicilanB, OUTSTANDING_P=$arrOutstandingP, OUTSTANDING_B=$arrOutstandingB, LAST_UPDATED_BY=$emp_id, LAST_UPDATED_DATE=SYSDATE WHERE APP_ID=" . APPCODE . " AND PERSON_ID=$person_id ";
		$result = oci_parse($con,$sqlQuery);
		oci_execute($result);
