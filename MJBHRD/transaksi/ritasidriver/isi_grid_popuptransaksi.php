...
if (isset($_GET['hdid'])){
	...
	if($hdid != ''){
		...
		, CASE WHEN REGEXP_SUBSTR( REPLACE(MSB.JAM_BERANGKAT, ';', ':'), '[^:]+', 1, 1 ) < 7 THEN TO_CHAR(MSB.TANGGAL_SJ-1, 'DD-MON-YYYY') ELSE TO_CHAR(MSB.TANGGAL_SJ, 'DD-MON-YYYY') END TANGGL_SJ
		, MSB.JAM_BERANGKAT
		, HOU.NAME PLANT
		, HOU.ATTRIBUTE2 TIPE
		, MSB.NAMA_CUST
...
, MSB.SURAT_JALAN_NO
--, TO_CHAR(MSB.TANGGAL_SJ, 'DD-MON-YYYY') TANGGL_SJ
, CASE WHEN REGEXP_SUBSTR( REPLACE(REPLACE(TRIM(MSB.JAM_BERANGKAT), ';', ':'), '.', ''), '[^:]+', 1, 1 ) < 7 THEN TO_CHAR(MSB.TANGGAL_SJ-1, 'DD-MON-YYYY') ELSE TO_CHAR(MSB.TANGGAL_SJ, 'DD-MON-YYYY') END TANGGL_SJ
, MSB.JAM_BERANGKAT
, HOU.NAME PLANT
, HOU.ATTRIBUTE2 TIPE
, MSB.NAMA_CUST
