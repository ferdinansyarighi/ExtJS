 ...
 if(isset($_POST['nama_user']))
  {
	...
	// mode edit record
		...
		$resultCek = oci_parse($con,"SELECT * from MJ.MJ_T_PINJAMAN where id = $hdid and STATUS_DOKUMEN <> 'Approved'
			AND TINGKAT = 0"); 
		...
		if($cek == ''){
			...
			WHERE APP_ID=" . APPCODE . " AND ID=$hdid ";
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
			...
			$queryUpload = "DELETE FROM MJ.MJ_M_UPLOAD WHERE APP_ID=" . APPCODE . " AND USERNAME='$user_id' AND TRANSAKSI_KODE='PINJAMAN' AND TRANSAKSI_ID=$hdid";
			//echo $query;
			$resultUpload = oci_parse($con,$queryUpload);
			oci_execute($resultUpload);
