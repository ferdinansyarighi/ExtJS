...
if ($jumlah>0)
{
	$data = "Nama Driver sudah pernah diinput.";
} else {
	if($typeform=="tambah"){
		//insert Header
		...
		$hdid = $row[0];
		$sqlQuery = "INSERT INTO MJ.MJ_T_RITASI_DRIVER (ID, NAMA_DRIVER, STATUS, CREATED_BY, CREATED_DATE) 
		VALUES ( $hdid, $nama, '$typetrans', '$user_id', SYSDATE)";
		//echo $sqlQuery;
		...
		for ($x=0; $x<$countID; $x++){
			...
			if($noSJ == 'STANDBY'){
				...
				AND SJ_ID = $TransID
				AND TO_CHAR(TGL_STANDBY, 'DD-MON-YYYY') = '$tglSJ'
				");
				oci_execute($result);
				$rowJum = oci_fetch_row($result);
				$jumlah = $rowJum[0];
			...
			if ($jumlah > 0){
				if($tglSJ == ''){
					...
					$sqlQuery = "UPDATE MJ.MJ_T_RITASI_DRIVER_DETAIL SET NOMOR_LHO = '$noLHO', KETERANGAN = '$ketHRD', TIPE = '$tipe', KM_AWAL = '$kmAwal', KM_AKHIR = '$kmAkhir', SOLAR = '$solar', VARIABEL = '$variabel', NOMINAL = '$nominal', NOMINAL_ID = $nominalID, GROUP_ID = $groupID, RITASI_KE = $ritasiKe, TGL_STANDBY='$tglSJ', LAST_UPDATED_BY='$user_id', LAST_UPDATED_DATE=SYSDATE
					WHERE MJ_T_RITASI_DRIVER_ID = $hdid AND SJ_ID = $TransID AND TO_CHAR(TGL_STANDBY, 'DD-MON-YYYY') = '$tglSJ' ";
					//echo $sqlQuery;
					$result = oci_parse($con,$sqlQuery);
					oci_execute($result);
				}
		...
		oci_execute($result);
		if($kumpulanTgl != ''){
			$sqlQuery = "DELETE FROM MJ.MJ_T_RITASI_DRIVER_DETAIL WHERE MJ_T_RITASI_DRIVER_ID = $hdid AND SJ_ID = 0 AND TO_CHAR(TGL_STANDBY, 'DD-MON-YYYY') not in($kumpulanTgl) ";
			//echo $sqlQuery;exit;
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
		}
