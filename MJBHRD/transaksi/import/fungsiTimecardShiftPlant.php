		...
		if($countData >= 1){
			...
			if($countData >= 1){
				...
				$cekShiftGanti = "SELECT COUNT(-1) 
					    ...
					    WHERE PAF.PERSON_ID = 4758 
					    AND PAF.ASSIGNMENT_ID = MCS.ASSIGNMENT_ID 
					    AND MCS.DATE_DETAIL = TO_DATE(SYSDATE-3)";
				$resultCekGanti = oci_parse($con,$queryShiftGanti);
				...
				if($CekGanti >= 1)
				{
					$queryCountMasuk = "SELECT COUNT(-1)
						    ...
						    , APPS.HXT_SHIFTS F
						    WHERE A.PERSON_ID=4758 AND A. EFFECTIVE_END_DATE > SYSDATE
						    AND A.PERSON_ID=B.PERSON_ID AND B. EFFECTIVE_END_DATE > SYSDATE
						    AND B.ASSIGNMENT_ID=C.ASSIGNMENT_ID AND C. DATE_DETAIL = TO_DATE(SYSDATE-3)
						    AND C.SHIFT_ID=D.TWS_ID AND D.START_DATE < SYSDATE 
						    AND D.START_DATE=(SELECT MAX(D.START_DATE) FROM APPS.PER_PEOPLE_F A, APPS.PER_ALL_ASSIGNMENTS_F B, MJ.MJ_M_CHANGE_SHIFT C, APPS.HXT_ROTATION_SCHEDULES D
						                        WHERE D.START_DATE < SYSDATE AND C.SHIFT_ID = D.TWS_ID
						                        AND A.PERSON_ID = 4758 AND A. EFFECTIVE_END_DATE > SYSDATE
						                        AND A.PERSON_ID = B.PERSON_ID AND B. EFFECTIVE_END_DATE > SYSDATE
						                        AND B.ASSIGNMENT_ID = C.ASSIGNMENT_ID AND C. DATE_DETAIL = TO_DATE(SYSDATE-3)
						                        AND C.SHIFT_ID = D.TWS_ID AND D.START_DATE < SYSDATE )
						    AND D.TWS_ID=E.TWS_ID AND UPPER(E.MEANING)='FRIDAY'-- Hari absen
						    AND E.SHT_ID=F.ID";
						...
						$queryCountMasuk = "SELECT COUNT(-1)
								...
								WHERE A.PERSON_ID=4758 AND A. EFFECTIVE_END_DATE > SYSDATE
								AND A.PERSON_ID=B.PERSON_ID AND B. EFFECTIVE_END_DATE > SYSDATE
								AND B.ASSIGNMENT_ID=C.ASSIGNMENT_ID AND C. DATE_FROM >= SYSDATE AND C.DATE_TO <= SYSDATE
								AND C.SHIFT_ID=D.TWS_ID AND D.START_DATE < SYSDATE 
								AND D.START_DATE=(SELECT MAX(D.START_DATE) FROM APPS.PER_PEOPLE_F A, APPS.PER_ALL_ASSIGNMENTS_F B, MJ.MJ_M_CHANGE_SHIFT C, APPS.HXT_ROTATION_SCHEDULES D
								                  WHERE D.START_DATE < SYSDATE AND C.SHIFT_ID = D.TWS_ID
								                  AND A.PERSON_ID = 4758 AND A. EFFECTIVE_END_DATE > SYSDATE
