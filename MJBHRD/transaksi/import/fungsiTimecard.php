<?PHP
function InsertTimeCard($con, $dataId, $newformat, $JamMasukAsli, $JamMasuk, $JamKeluarAsli, $JamKeluar, $user_id, $pengurang){
	$vID = 0;
	//utk mengecek karyawan sudah memiliki honors 
	...
	$queryCekAbsen = "SELECT COUNT(-1) FROM MJ.MJ_T_TIMECARD WHERE PERSON_ID = $vID AND TANGGAL = TO_DATE('$newformat', 'YYYY-MM-DD') AND CREATED_BY = 24";
	...
	//echo $newformat; exit;
	if($countAbsen >= 1){
		$queryTimeCard = "DELETE MJ.MJ_T_TIMECARD WHERE PERSON_ID = $vID AND TANGGAL = TO_DATE('$newformat', 'YYYY-MM-DD') AND CREATED_BY = 24";
		$result = oci_parse($con,$queryTimeCard);
		oci_execute($result);
	}

	//kalau honors/id finger belum ada maka absen tidak masuk pada oracle
	if($vID >= 1){
		//cek apakah sudah resign apa belum
		$queryDataId = "SELECT COUNT(-1) FROM APPS.PER_PEOPLE_F WHERE PERSON_ID=$dataId AND EFFECTIVE_END_DATE > SYSDATE-$pengurang AND NVL(DATE_EMPLOYEE_DATA_VERIFIED, TO_DATE('12/31/4712', 'MM/DD/YYYY')) > SYSDATE-$pengurang";
		...
		//echo $newformat; exit;
		if($countData >= 1){
			//Cek apakah sudah aktif masuk merak
			...
			if($countData >= 1){
				...
				//Cek HOURS / shift apakah sudah dibuatkan apa belum
				...
				// echo $HariIni . '<BR>' . $HariLibur . '<BR>' . $JamKeluarAsli . '<BR>' . $JamMasukAsli . '<BR>' . $CountMasuk. '<BR>' . $Keluar;exit;
				//Cek apakah libur apa nggak
				if($HariIni != 'SUNDAY' && $HariLibur == 0){
					//Cek jam masuk dan jam keluar
					if ($JamKeluarAsli=='' || $JamMasukAsli==''){
						if($CountMasuk >= 1){
							//Cek ada SIK apa nggak
							...
							if($CountKategori >= 1){
								...
								if($Kategori == 'Cuti'){
									...
									if($IjinKhusus == 'SETENGAH HARI'){
										...
										$xJamKeluarAsli = $vTempJamKeluar . ":" . $vTempMenitKeluar;
										if ($JamMasukAsli==''){
											$zJamMasuk = str_replace(':', '', $xJamMasukAsli);
											if($zJamMasuk >= $Terlambat){
										...
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE, DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$xJamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
										if ($JamMasukAsli==''){
											$JamMasukAsli = $xJamKeluarAsli;
										} else {
											$JamKeluarAsli = $xJamKeluarAsli;
										}
									}
								} else {
								...
								if ($JamMasukAsli != ''){
									if($JamMasuk >= $Terlambat){
										...
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE,DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$oJamMasukAsli', '$JamMasukAsli', '$oStatus', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
									}
								}
							...
							if ($Masuk != '') { 
								...
								if($JamKeluar > $Keluar){
									...
									if($tempJam >= 1){
										...
										$rowHSeq = oci_fetch_row($resultSeq);
										$IdTimecard = $rowHSeq[0];
										//echo $xJamMasukAsli;exit;
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE,DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$JamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
									}
								}
								...
								$rowHSeq = oci_fetch_row($resultSeq);
								$IdTimecard = $rowHSeq[0];
								$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE,DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$JamMasukAsli', '$JamKeluarAsli', '$Status', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
								$result = oci_parse($con,$queryTimeCard);
								oci_execute($result);
							} else {
								if ($JamMasukAsli==''){
						...
						if($CountMasuk >= 1){
							if ($Masuk != '') {
								if($JamMasuk >= $Terlambat){
									...
									$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE,DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamMasukAsli', '$JamMasukAsli', '$xStatus', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
									$result = oci_parse($con,$queryTimeCard);
									oci_execute($result);
								} else {
									$Status=StatusTimecard($con, 'MASUK'); //Masuk
								...
								if($JamKeluar > $Keluar){
									...
									if($tempJam >= 1){
										...
										$queryTimeCard = "INSERT INTO MJ.MJ_T_TIMECARD (ID, APP_ID, PERSON_ID, TANGGAL, JAM_MASUK, JAM_KELUAR, STATUS, STATUS_SYNC, CREATED_BY, CREATED_DATE,DATE_IN,DATE_OUT) VALUES ($IdTimecard, " . APPCODE . ", $dataId, TO_DATE('$newformat', 'YYYY-MM-DD'), '$xJamKeluarAsli', '$JamKeluarAsli', '$xStatus', 'Belum', '$user_id', SYSDATE,TO_DATE('$newformat', 'YYYY-MM-DD'),TO_DATE('$newformat', 'YYYY-MM-DD'))";
										$result = oci_parse($con,$queryTimeCard);
										oci_execute($result);
									}
								}
