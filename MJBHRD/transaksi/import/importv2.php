...
FROM MastKarya MK --WHERE MK.KaryaCode=10567";
...
while(odbc_fetch_row($resultHeader)){
	...
	$Tanggal       = odbc_result($resultHeader, 2);
	$TglHeader     = substr(odbc_result($resultHeader, 2), 0, 10);

	$JamMasukAsli  = '';
	$JamKeluarAsli = '';
	$countIn       = 0;
	$queryKar = "SELECT DISTINCT PPF.FULL_NAME, PPF.PERSON_ID FROM PER_PEOPLE_F PPF WHERE HONORS = '$dataId'";
	$resultKar = oci_parse($con, $queryKar);
	oci_execute($resultKar);
	$rowKar = oci_fetch_row($resultKar);
	$NameKar=$rowKar[0];
	$PeridKar=$rowKar[1];
	...
	while(odbc_fetch_row($result)){
		...
		$querycek = oci_parse($con, "SELECT COUNT(-1) FROM MJ.MJ_TEMP_BIO_HO WHERE HONORS = $dataId AND TANGGALABSEN = '$Tgl' AND STATUS = $Status");
		oci_execute($querycek);
		$rowcek   = oci_fetch_row($querycek);
		$cek = $rowcek[0];
		//echo $cek;exit;
		// JIKA DATA BELUM ADA PADA TEMPORARY
		if($cek==0){
			$resultSeq = oci_parse($con,"SELECT MJ.MJ_TEMP_BIO_HO_SEQ.nextval FROM DUAL");
			oci_execute($resultSeq);
			$rowHSeq = oci_fetch_row($resultSeq);
			$gencountseq = $rowHSeq[0];

			$resultabsen = oci_parse($con,"INSERT INTO MJ.MJ_TEMP_BIO_HO (ID, HONORS, PERSON_ID, FULL_NAME, TANGGAL, TANGGALABSEN, STATUS,CREATED_BY,CREATED_DATE) VALUES ($gencountseq, $dataId, $PeridKar,'$NameKar',TO_DATE('$Tanggal','YYYY-MM-DD'),'$Tgl',$Status,24,SYSDATE)");
			oci_execute($resultabsen); 
		}
	}
}
//exit;
...
$vSdate = $rowDate[0];
	...
	$queryHeaderf = "SELECT DISTINCT MTB.HONORS, TO_CHAR(SYSDATE-$pengurang,'YYYY-MM-DD')
	FROM MJ.MJ_TEMP_BIO_HO MTB--WHERE MTB.HONORS=10567";
	$resultf = oci_parse($con,$queryHeaderf);
	oci_execute($resultf);

	// PERULANGAN UNTUK MEMBUAT FLAG DATA SEBANYAK DATA YANG BARU MENCAPAI 80%
   while($rowf = oci_fetch_row($resultf)){
		$dataId 		  = $rowf[0];
		$TglHeader 		  = $rowf[1]; 
		$JamMasukAsli 	  = '';
		$JamKeluarAsli    = '';
		...
		if($CekGanti == 0)
		{ 
			$querywow = "SELECT MTB.HONORS, MTB.TANGGALABSEN, MTB.STATUS
			FROM MJ.MJ_TEMP_BIO_HO MTB
			WHERE TO_CHAR(TANGGAL, 'YYYY-MM-DD') = TO_CHAR(SYSDATE-$pengurang, 'YYYY-MM-DD')
			AND MTB.HONORS=$dataId ";
			$resultwow = oci_parse($con,$querywow);
			oci_execute($resultwow);

			// PERULANGAN UNTUK MEMBUAT FLAG DATA SEBANYAK DATA YANG BARU MENCAPAI 80%
			while($rowwow = oci_fetch_row($resultwow)){
				$Tgl 		= $rowwow[1]; 
				$Status 	= $rowwow[2]; 
