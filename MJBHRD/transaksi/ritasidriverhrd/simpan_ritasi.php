 ...
 if(isset($_POST['typeform']))
  {
	...
	$arrIDdt=json_decode($_POST['arrIDdt']);
	...
	if($typeform=="tambah"){
		...
		if(!oci_execute($result)){
			...
			for ($x=0; $x<$countID; $x++){
				...
				$sqlQuery = "UPDATE MJ.MJ_T_RITASI_DRIVER_DETAIL SET MJ_T_RITASI_DRIVER_ID = $hdid, NOMOR_LHO = $arrLHO[$x] , KETERANGAN = '$ketHRD', TIPE = '$tipe', KM_AWAL = $kmAwal, KM_AKHIR = $kmAkhir, SOLAR = $solar, VARIABEL = $variabel, RITASI_KE = $ritasiKe, NOMINAL = '$nominal', NOMINAL_ID = $nominalID, GROUP_ID = $groupID WHERE ID = $TransIDdt ";
		...
		for ($x=0; $x<$countID; $x++){
			...
			if ($jumlah > 0){
				...
				WHERE MJ_T_RITASI_DRIVER_ID = $hdid AND ID = $TransIDdt ";
				//echo $sqlQuery;
				$result = oci_parse($con,$sqlQuery);
				oci_execute($result);
			} else {				
		...
		$result = oci_parse($con, "select id from MJ.MJ_T_RITASI_DRIVER
			where nama_driver = $nama
			and status = 'Save'
		...
		$id_header_admin = $row[0]; 
		
		if($id_header_admin != ''){
			
			$sqlQuery = "update MJ.MJ_T_RITASI_DRIVER_detail set MJ_T_RITASI_DRIVER_ID = $id_header_admin where MJ_T_RITASI_DRIVER_ID = $hdid AND ID NOT IN ($kumpulanID) ";
			//echo $sqlQuery;exit;
			//$sqlQuery = "DELETE FROM MJ.MJ_T_RITASI_DRIVER_DETAIL WHERE MJ_T_RITASI_DRIVER_ID = $hdid AND ID NOT IN ($kumpulanID) ";
			//echo $sqlQuery;
			$result = oci_parse($con,$sqlQuery);
			oci_execute($result);
		}
