 ...
 if(isset($_POST['hdid']))
  {
	...
	if($typeform == "Approved"){
		$sqlQueryA = "UPDATE MJ.MJ_T_PINJAMAN SET KETERANGAN = '$keterangan', TINGKAT = 2 WHERE ID = $hdid";
		$resultA = oci_parse($con,$sqlQueryA);
		oci_execute($resultA);
	} else {
		$sqlQueryA = "UPDATE MJ.MJ_T_PINJAMAN SET KETERANGAN = '$keterangan', TINGKAT = 0, STATUS_DOKUMEN = '$typeform' WHERE ID = $hdid";
		$resultA = oci_parse($con,$sqlQueryA);
		oci_execute($resultA);
	}
	...
	$query = "
	SELECT FILENAME, FILESIZE, FILETYPE, TRANSAKSI_KODE
