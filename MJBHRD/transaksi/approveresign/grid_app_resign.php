...
{	
	...
	if ($tglfrom!=''){
		$queryfilter .=" AND TGL_RESIGN >= TO_DATE('$tglfrom', 'YYYY/MM/DD') ";
	}
	if ($tglto!=''){
		$queryfilter .=" AND TGL_RESIGN <= TO_DATE('$tglto', 'YYYY/MM/DD') ";
	}
	if ($nopengajuan!=''){
...
if($jumgen == 0)
{
	...
	FROM MJ.MJ_T_RESIGN WHERE MANAGER LIKE '%$emp_name%' AND APPROVAL_MANAGER LIKE 'In Proccess' AND (VALIDASI IS NULL OR VALIDASI='') 
		AND STATUS = 1
	$queryfilter";
	//echo $query;exit;
}
	...
	BULAN_LAMA_KERJA, HARI_LAMA_KERJA, MANAGER, KETERANGAN, STATUS, APPROVAL_MANAGER, APPROVAL_MANAGER_HRD, TGL_APP_TERAKHIR, CREATED_BY,CREATED_DATE, 
	TAHUN_LAMA_KERJA || ' Tahun ' || BULAN_LAMA_KERJA || ' Bulan ' || HARI_LAMA_KERJA || ' Hari' LAMA_KERJA, KETERANGAN_DISAPPROVE, TGL_PENGAJUAN
	FROM MJ.MJ_T_RESIGN WHERE APPROVAL_MANAGER LIKE 'Approved' AND APPROVAL_MANAGER_HRD = 'In Proccess'
	 AND (VALIDASI IS NULL OR VALIDASI='') AND STATUS = 1 $queryfilter
	UNION
	SELECT ID,NO_PENGAJUAN,NAMA_KARYAWAN,COMPANY,DEPARTMENT,POSITION,GRADE,LOCATION,TGL_MASUK,TGL_RESIGN,TAHUN_LAMA_KERJA,
	BULAN_LAMA_KERJA, HARI_LAMA_KERJA, MANAGER, KETERANGAN, STATUS, APPROVAL_MANAGER, APPROVAL_MANAGER_HRD, TGL_APP_TERAKHIR, CREATED_BY,CREATED_DATE, 
	TAHUN_LAMA_KERJA || ' Tahun ' || BULAN_LAMA_KERJA || ' Bulan ' || HARI_LAMA_KERJA || ' Hari' LAMA_KERJA, KETERANGAN_DISAPPROVE, TGL_PENGAJUAN
	FROM MJ.MJ_T_RESIGN WHERE MANAGER LIKE '%$emp_name%' AND APPROVAL_MANAGER = 'In Proccess' AND APPROVAL_MANAGER_HRD = 'In Proccess'
	AND (VALIDASI IS NULL OR VALIDASI='') AND STATUS = 1 $queryfilter";
}
