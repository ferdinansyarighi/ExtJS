...
if(isset($_POST['nama_pem'])){
	...
	$query_pencairan = "
		SELECT  TIPE_PENCAIRAN, NO_REK, SYARAT1, SYARAT2, SYARAT3, SYARAT4, SYARAT5
		FROM    MJ_T_BS
		WHERE   ID = $hdid
	";
	
	$resultPencairan = oci_parse( $con, $query_pencairan );
	oci_execute( $resultPencairan );
	$rowPencairan = oci_fetch_row( $resultPencairan );
	
	$tipe_pencairan = $rowPencairan[0];
	$no_rek = $rowPencairan[1];
	
	$syarat_a = $rowPencairan[2];
	$syarat_b = $rowPencairan[3];
	$syarat_c = $rowPencairan[4];
	$syarat_d = $rowPencairan[5];
	$syarat_e = $rowPencairan[6];
	
	$hasil = $perusahaan . "|" . $departement . "|" . $posisi . "|" . $plant . "|" . $bank . "|" . $noreg . "|" . $an . "|" .$grade . "|" . $rek
				. "|" . $tipe_pencairan . "|" . $no_rek 
				. "|" . $syarat_a . "|" . $syarat_b . "|" . $syarat_c . "|" . $syarat_d . "|" . $syarat_e;
