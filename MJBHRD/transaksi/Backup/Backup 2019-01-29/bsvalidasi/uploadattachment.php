...
if (is_uploaded_file($tmpname))
{
	...
	$queryCek = "SELECT COUNT(-1) FROM MJ.MJ_TEMP_UPLOAD WHERE FILENAME = '$file_name' and FILETYPE = '$file_type' and TRANSAKSI_KODE = 'BONKASIR' AND USERNAME='$user_id'";
		...
		if($file_size <= 512000){
			...
			if(!move_uploaded_file($tmpname,"../../upload/bs/".$file_fullnewname)){ //success not upload
				...
				$sqlString 	= "	INSERT INTO MJ.MJ_TEMP_UPLOAD(ID, APP_ID, TRANSAKSI_ID, FILENAME, FILESIZE, FILETYPE, USERNAME, CREATEDDATE, TRANSAKSI_KODE) 
								VALUES($id, " . APPCODE . ", '1', '".$file_name."', '".$arsipSize."', '".$arsipType."', '".$userid."', TO_DATE('$date1', 'YYYY-MM-DD'), 'BONKASIR')";
								//echo $sqlString;
