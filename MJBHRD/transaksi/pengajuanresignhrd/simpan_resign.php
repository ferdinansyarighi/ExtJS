...
if ($formtype=='tambah')
{
	...
	$querycountEx = "SELECT COUNT(-1) FROM MJ.MJ_T_RESIGN WHERE NAMA_KARYAWAN = '$nama_karyawan' AND (APPROVAL_MANAGER = 'In Proccess' OR APPROVAL_MANAGER_HRD = 'In Proccess') AND STATUS = 1";
	...
	$querycountExe = "SELECT COUNT(-1) FROM MJ.MJ_T_RESIGN WHERE NAMA_KARYAWAN = '$nama_karyawan' AND APPROVAL_MANAGER = 'Approved' AND APPROVAL_MANAGER_HRD = 'In Proccess' AND BY_HRD = 'Y' AND STATUS = 1";
	...
	**/
	...
	else 
	{
		...
		$result = oci_parse($con,"INSERT INTO MJ.MJ_T_RESIGN (ID, NO_PENGAJUAN, NAMA_KARYAWAN, COMPANY, DEPARTMENT, POSITION, GRADE, LOCATION, TGL_MASUK, TGL_RESIGN, TGL_PENGAJUAN, TAHUN_LAMA_KERJA, BULAN_LAMA_KERJA, HARI_LAMA_KERJA, MANAGER, KETERANGAN, STATUS, APPROVAL_MANAGER, APPROVAL_MANAGER_HRD, TGL_APP_TERAKHIR, CREATED_BY, CREATED_DATE, BY_HRD) 
		VALUES ($gencountseq, '$no_pengajuan', '$nama_karyawan','$company', '$department', '$position', '$grade', '$location', TO_DATE('$tglmasuk', 'DD/MM/YY'), TO_DATE('$tglresign', 'DD/MM/YY'), TO_DATE('$tglpengajuan', 'DD/MM/YY'), '$tahunkerja', '$bulankerja', '$harikerja', '$manager', '$keterangan', '$status', 'Approved', 'In Proccess', sysdate, $emp_id, sysdate, 'Y')");
		oci_execute($result); 
...
Terima Kasih,";
	...
	$q_check_mgr = "SELECT COUNT(-1) FROM MJ.MJ_T_RESIGN
					WHERE ID = $hdid AND APPROVAL_MANAGER_HRD = 'Approved' ";
...
/*	$querycountDat = "SELECT DISTINCT COUNT(-1) FROM MJ.MJ_T_RESIGN MTR, PER_PEOPLE_F PPF, PER_ASSIGNMENTS_F PAF, PER_POSITIONS PP
	...
	OR (MTR.ID = $hdid AND MTR.APPROVAL_MANAGER = 'Approved' AND MTR.APPROVAL_MANAGER_HRD = 'In Proccess' AND BY_HRD = 'Y')";
	...
	// Kalau data tersebut belum pernah dibuat
/*	if($jumdat == 0)
	{
		//count data sesuai hdid dan approval manager disapprove
		$queryhehe = "SELECT COUNT(-1) FROM MJ.MJ_T_RESIGN WHERE ID = $hdid AND APPROVAL_MANAGER = 'Approved' AND APPROVAL_MANAGER_HRD = 'Disapproved' AND BY_HRD = 'Y'";
...
/*		$query = "DELETE FROM MJ.MJ_M_UPLOAD
		...
		$result = oci_parse($con,"UPDATE MJ.MJ_T_RESIGN SET TGL_PENGAJUAN = TO_DATE('$tglpengajuan', 'DD/MM/YY'),
									TGL_RESIGN = TO_DATE('$tglresign', 'DD/MM/YY'), TAHUN_LAMA_KERJA = '$tahunkerja',
									BULAN_LAMA_KERJA = '$bulankerja', HARI_LAMA_KERJA = '$harikerja', MANAGER = '$manager',
									KETERANGAN = '$keterangan', STATUS = '$status', LAST_UPDATED_DATE = sysdate ,
									LAST_UPDATED_BY = $emp_id, APPROVAL_MANAGER_HRD = 'In Proccess' WHERE ID = $hdid");
		oci_execute($result); 
