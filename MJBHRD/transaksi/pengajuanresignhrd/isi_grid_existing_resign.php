<?php
	...
	$query = "SELECT ID, NO_PENGAJUAN, NAMA_KARYAWAN, COMPANY, DEPARTMENT, POSITION, GRADE, LOCATION, TO_CHAR(TGL_MASUK, 'DD/MM/YYYY'),
					TO_CHAR(TGL_RESIGN, 'DD/MM/YYYY'), TAHUN_LAMA_KERJA, BULAN_LAMA_KERJA, HARI_LAMA_KERJA, MANAGER, KETERANGAN, STATUS,
					APPROVAL_MANAGER, APPROVAL_MANAGER_HRD, TGL_APP_TERAKHIR, CREATED_BY,CREATED_DATE, TO_CHAR(TGL_PENGAJUAN, 'DD/MM/YYYY')
				FROM MJ.MJ_T_RESIGN
				WHERE (CREATED_BY = $emp_id OR NAMA_KARYAWAN LIKE '$emp_name') AND STATUS = 1 AND (VALIDASI IS NULL OR VALIDASI='')";
