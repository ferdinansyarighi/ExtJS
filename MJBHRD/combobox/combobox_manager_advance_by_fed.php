...
if($jumgen >= 1) // JIKA HRD LOGIN (TERMASUK STAFF DAN MANAGER)
{
	$query = "SELECT DISTINCT PPF.FULL_NAME AS DATA_VALUE, PPF.FULL_NAME AS DATA_NAME FROM PER_PEOPLE_F PPF, PER_POSITIONS PP, PER_ASSIGNMENTS_F PAF 
	WHERE PAF.PERSON_ID = PPF.PERSON_ID AND PAF.POSITION_ID = PP.POSITION_ID AND PP.NAME LIKE '%MGR%' AND PPF.FULL_NAME NOT LIKE '%TRIAL%' 
	AND PPF.FULL_NAME NOT LIKE '%SALAH%' $querywhere";
}
else if ($jumman >= 1) // JIKA MANAGER SELAIN HRD LOGIN
{
	...
	WHERE PAF.PERSON_ID = PPF.PERSON_ID AND PAF.POSITION_ID = PP.POSITION_ID AND PP.NAME LIKE '%MGR%' AND PPF.FULL_NAME NOT LIKE '%TRIAL%' 
	AND PPF.FULL_NAME NOT LIKE '%SALAH%' AND PAF.JOB_ID = $jobPemohon $querywhere";	
}
else { // JIKA STAFF SELAIN HRD
	$query = "SELECT DISTINCT PPF.FULL_NAME AS DATA_VALUE, PPF.FULL_NAME AS DATA_NAME 
	FROM PER_PEOPLE_F PPF, PER_POSITIONS PP, PER_ASSIGNMENTS_F PAF
	WHERE PAF.PERSON_ID = PPF.PERSON_ID AND PAF.POSITION_ID = PP.POSITION_ID AND PP.NAME LIKE '%MGR%' AND PPF.FULL_NAME NOT LIKE '%TRIAL%' 
	AND PPF.FULL_NAME NOT LIKE '%SALAH%' AND PAF.JOB_ID = $jobPemohon $querywhere";
