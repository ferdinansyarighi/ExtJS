...
AND UPPER(PPF.FULL_NAME) LIKE '%$pemohon%' AND paf.PRIMARY_FLAG = 'Y'
...
INNER JOIN APPS.PER_POSITIONS PP ON PP.POSITION_ID=PAF.POSITION_ID
WHERE PPF.EFFECTIVE_END_DATE > SYSDATE AND PAF.EFFECTIVE_END_DATE > SYSDATE 
AND (PP.NAME LIKE '0.%' OR PP.NAME LIKE '%SPV%' OR PP.NAME LIKE '%SPI%') $querywhere 
AND 0 = $count
--AND  UPPER(PPF.FULL_NAME) NOT LIKE '%$pemohon%'
