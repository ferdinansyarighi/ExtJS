...
// $queryLogin = "SELECT CASE WHEN PP.NAME LIKE '%DIR%' THEN '2'
                // WHEN PP.NAME LIKE '%MGR%FIN%' THEN '3'
...
SELECT DISTINCT BS.ID, BS.NO_BS, PPF.FULL_NAME PEMOHON, HOU.NAME PERUSAHAAN
		...
		, CASE WHEN BS.TINGKAT = 0 AND BS.STATUS = 'PROCESS' THEN PPF2.FULL_NAME
			...
			WHEN BS.TINGKAT = 5 AND BS.STATUS = 'Approved' 
			and BS.TGL_PENCAIRAN IS NULL THEN 'Validasi Kasir' 
...
LEFT JOIN APPS.PER_PEOPLE_F PPF3 ON BS.LAST_UPDATED_BY = PPF3.PERSON_ID
LEFT JOIN MJ.MJ_T_APPROVAL MTA ON BS.ID = MTA.TRANSAKSI_ID AND MTA.TRANSAKSI_KODE = 'BON' AND BS.TINGKAT = MTA.TINGKAT
WHERE 1=1 
AND ( BS.ASSIGNMENT_ID = $assignment_id_login OR BS.CREATED_BY = $emp_id )
$queryfilter
ORDER BY BS.ID";

// -- and BS.ASSIGNMENT_ID = $assignment_id_login  
...
while($row = oci_fetch_row($result))
{
	...
	$record['DATA_APP_TERAKHIR']=$row[13];
	$record['DATA_TGL_TERAKHIR']=$row[14];
	$record['DATA_KET_DISAPP']=$row[15];
	$record['DATA_NEXT_APP']=$row[16];
	$record['DATA_TIPE_BS']=$row[17];
	$record['DATA_TANGGAL_PENCAIRAN']=$row[17];
	$record['DATA_TANGGAL_CLOSE']=$row[18];
	$record['DATA_NOMINAL_OUTSTANDING']=$row[19];
