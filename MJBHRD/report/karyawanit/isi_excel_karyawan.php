...
<tr>
    ...
    <td><div align=\"center\">Jam Masuk Biofinger</div></td>
    <td><div align=\"center\">Jam Keluar Biofinger</div></td>
    <td><div align=\"center\">Tgl Insert Biofinger</div></td>
    <td><div align=\"center\">Keterangan</div></td>
    <td><div align=\"center\">SIK / SPL</div></td>
    <td><div align=\"center\">Ijin Khusus</div></td>
    <td><div align=\"center\">Alasan</div></td>
 ...
 ";
	...
	while($row = oci_fetch_row($resultHeader))
	{
		...
		if($honors != ''){
			$query = "SELECT CIO.KaryaCode, CIO.TranDate, CIO.StateCode, CONVERT(VARCHAR(10), CIO.UserDate,120)
			FROM CardInOut CIO 
			WHERE CONVERT(VARCHAR(10), CIO.TranDate,120)='$tanggalAbsen'
			AND CIO.KaryaCode=$honors ";
			$result = odbc_exec($conn, $query);
			while(odbc_fetch_row($result)){
				$Tgl = odbc_result($result, 2);
				$Status = odbc_result($result, 3);
				$userDate = odbc_result($result, 4);

				if($Status == 0 && $countIn == 0){
					$JamMasukBio = substr($Tgl, 11, 5);
					//$JamMasuk = str_replace(':', '', $JamMasukAsli);
					$countIn++;
				}
				if($Status == 1){
					$JamKeluarBio = substr($Tgl, 11, 5);
		...
		<tr>
			...
			<td><div align=\"left\">$JamMasukBio</div></td>
			<td><div align=\"left\">$JamKeluarBio</div></td>
			<td><div align=\"left\">$userDate</div></td>
			<td><div align=\"left\">$keterangan</div></td>
			<td><div align=\"left\">$sikspl</div></td>
			<td><div align=\"left\">$ijinKhusus</div></td>
			<td><div align=\"left\">$tglAlasan</div></td>
