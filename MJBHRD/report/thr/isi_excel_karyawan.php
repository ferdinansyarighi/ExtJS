...
{	
	...
	$dept = $_GET['dept'];
	if($nama!=''){
		$queryfilter .= " AND NAMA_KARYAWAN='$nama'";
	}
	if($dept!=''){
...
<tr>
 ...
 ";
	...
	(
		SELECT DISTINCT PPF.FULL_NAME AS NAMA_KARYAWAN
		...
		INNER JOIN APPS.PER_ALL_ASSIGNMENTS_F PAF ON PPF.PERSON_ID=PAF.PERSON_ID AND PAF.EFFECTIVE_END_DATE > SYSDATE
		LEFT JOIN APPS.PER_JOBS J ON PAF.JOB_ID=J.JOB_ID
		INNER JOIN MJ.MJ_M_ELEMENT MME ON MME.ELEMENT_ID=MTT.STATUS
		LEFT JOIN MJ.MJ_T_SIK MTS ON MTS.PERSON_ID=PPF.PERSON_ID AND MTT.TANGGAL BETWEEN MTS.TANGGAL_FROM AND MTS.TANGGAL_TO
		LEFT JOIN 
		(
		...
		UNION
		SELECT DISTINCT SIK.PEMOHON AS NAMA_KARYAWAN
		...
		INNER JOIN
		(
			SELECT SPLD.ID, SPL.TANGGAL_SPL, SPLD.STATUS_DOK, SPLD.NAMA, SPLD.JAM_FROM, SPLD.JAM_TO
			FROM MJ.MJ_T_SPL SPL
			INNER JOIN MJ.MJ_T_SPL_DETAIL SPLD ON SPL.ID=SPLD.MJ_T_SPL_ID
		) SPL ON SPL.NAMA=SIK.PEMOHON AND SPL.TANGGAL_SPL BETWEEN SIK.TANGGAL_FROM AND SIK.TANGGAL_TO
	) KARYAWAN
