...
SELECT MTM.ID
	...
	, to_char( NVL( MTA.CREATED_DATE, MTM.CREATED_DATE ), 'DD-MM-YYYY') TGL_APPR_MGR_LAMA
	...
	, MTM.JUMLAH_BULAN
FROM MJ.MJ_T_MUTASI MTM
LEFT JOIN APPS.PER_PEOPLE_F PPF ON PPF.PERSON_ID = MTM.KARYAWAN_ID AND PPF.EFFECTIVE_END_DATE > SYSDATE
LEFT JOIN APPS.PER_PEOPLE_F PPF1 ON PPF1.PERSON_ID = MTM.CREATED_BY AND PPF1.EFFECTIVE_END_DATE > SYSDATE
LEFT JOIN APPS.PER_PEOPLE_F PPF2 ON PPF2.PERSON_ID = MTM.DIREKSI AND PPF2.EFFECTIVE_END_DATE > SYSDATE
...
LEFT JOIN APPS.PER_GRADES PG ON MTM.GRADE_LAMA_ID = PG.GRADE_ID 
LEFT JOIN APPS.PER_GRADES PG1 ON MTM.GRADE_BARU_ID = PG1.GRADE_ID 
...
while($row = oci_fetch_row($result))
{
	...
	// Tambahan by Yuke on Sept 29, 2018
...
$managerHRD = $rowHRD[0];	
	...
	// Image "Approve" hanya untuk atasan lama dan atasan langsung dari atasan lama
	
	$pdf->Image('approve-min.png',20,220,25);
	...
	$pdf->Cell(120, 6, 'BASIC DATA', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, 'Lampiran Data Personal', 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 6, 'Position ID dan Initial', 1, 0, 'L', 0);
	$pdf->Cell(30, 6, $position_id, 1, 0, 'L', 0);
	...
	$pdf->Cell(30, 4, 'Status Karyawan *) :', 'L', 0, 'L', 0);
	$pdf->Cell(60, 4, '['.$skTetap.']  Tetap                       ['.$skPercobaan.']  Percobaan', 0, 0, 'L', 0);
	$pdf->Cell(30, 4, '['.$skPJS.']  PJS', 0, 0, 'L', 0);
	$pdf->Cell(60, 4, '['.$skKontrak.']  Kontrak                     ['.$skKhusus.']  Kontrak Khusus', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, 'Sifat Perubahan *) :', 'L', 0, 'L', 0);
	$pdf->Cell(60, 4, '['.$spTetap.']  Tetap                       ['.$spSementara.']  Sementara  '.$lama_bulan.'', 0, 0, 'L', 0);
	$pdf->Cell(30, 4, ' Bulan', 0, 0, 'L', 0);
	$pdf->Cell(60, 4, '', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, 'Jenis Perubahan *) :', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '['.$promosi.']  Promosi', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '[    ]  Rotasi', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '['.$demosi.']  Demosi', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '['.$mutasi.']  Mutasi', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '[    ]  Perpanjangan Kontrak', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '[    ]  Lolos Percobaan', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(60, 4, '[    ]  Lolos PJS                [    ]  Perpanjangan PJS', 0, 0, 'L', 0);
	$pdf->Cell(30, 4, '[    ]  Tidak Lolos PJS', 0, 0, 'L', 0);
	$pdf->Cell(60, 4, '', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, '', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '[    ]  Lain-lain', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(30, 4, 'Asal Perekrutan *) :', 'L', 0, 'L', 0);
	$pdf->Cell(150, 4, '[    ]  Subco lain **)', 'R', 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->SetFont('Arial', 'B', 7);
	$pdf->Cell(8, 5, '', 0, 0, 'L', 0);	
	
	
	// Keterangan perubahan diubah oleh Yuke di 3 Oktober 2018 atas permintaan ISA.
	
	// $pdf->Cell(180, 5, 'Keterangan Perubahan :  '.$keterangan, 'LBR', 0, 'L', 0);
	
	$pdf->Cell(180, 5, '', 'LBR', 0, 'L', 0);
	$pdf->Ln(5);
	...
	$pdf->Cell(60, 6, $perusahaan_lama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $perusahaan_baru, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Divisi', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, $divisi_lama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $divisi_baru, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Departemen', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, $xDeptLama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $xDept, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Cabang', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, $xLokasiLama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $xLokasi, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Bagian/Section', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, '', 1, 0, 'C', 0);
	$pdf->Cell(60, 6, '', 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Jabatan', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, $xPosisiLama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $xPosisi, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Jadwal Kerja', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, '', 1, 0, 'C', 0);
	$pdf->Cell(60, 6, '', 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	$pdf->Cell(60, 6, 'Pangkat', 1, 0, 'L', 0);
	$pdf->Cell(60, 6, $pangkat_lama, 1, 0, 'C', 0);
	$pdf->Cell(60, 6, $pangkat_baru, 1, 0, 'C', 0);
	$pdf->Ln(6);
	$pdf->Cell(8, 6, '', 0, 0, 'L', 0);	
	...
	$pdf->SetFont('Arial', 'B', 7);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(180, 4, ' *) Berikan tanda "Check List" pada kotak yang tersedia. Contoh : [ v ]', 0, 0, 'L', 0);
	$pdf->Ln(4);
	$pdf->Cell(8, 4, '', 0, 0, 'L', 0);	
	$pdf->Cell(180, 4, ' **) Bila perekrutan karyawan berasal dari Internal grup perusahaan (MJ Group).', 0, 0, 'L', 0);
	$pdf->Ln(4);
...
$pdf->Output( 'Mutasi_F-02' . $vNoReq . '_tingkat_1.pdf', 'F' );
