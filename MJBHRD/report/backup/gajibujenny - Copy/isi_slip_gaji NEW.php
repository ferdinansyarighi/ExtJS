...
$countHeader = 0;
	...
	while($row = oci_fetch_row($resultGaji))
	{
		...
		$queryJamLembur = "SELECT DISTINCT MTSD.TOTAL_JAM, NVL(MTT.JAM_MASUK, '16:00'), NVL(MTT.JAM_KELUAR, '23:59'), MTS.TANGGAL_SPL        
		FROM MJ.MJ_T_SPL MTS
		INNER JOIN MJ.MJ_T_SPL_DETAIL MTSD ON MTSD.MJ_T_SPL_ID=MTS.ID
		INNER JOIN MJ.MJ_T_TIMECARD MTT ON MTT.TANGGAL=MTS.TANGGAL_SPL AND MTT.STATUS=253 AND MTT.PERSON_ID=$vPersonId
		...
		while($rowJamLembur = oci_fetch_row($resultJamLembur))
		{
			...
			//echo $vTotSPL ." dan ". $vTempJamMasuk;
			if($vTotJamLembur < 25){
