...
{	
	...
	FROM MJ.MJ_M_GAJI WHERE REVISI=0 AND BULAN='$bulan' AND TAHUN='$tahun' ";
	//echo $queryJumHari;
	$resultCheckData = oci_parse($con,$queryCheckData);
	...
	$queryCheckData = "SELECT COUNT(-1)
	FROM MJ.MJ_M_GAJI WHERE REVISI=1 AND BULAN='$bulan' AND TAHUN='$tahun' ";
	//echo $queryJumHari;
	$resultCheckData = oci_parse($con,$queryCheckData);
	oci_execute($resultCheckData);
	$rowCheckData = oci_fetch_row($resultCheckData);
