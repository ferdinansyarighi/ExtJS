...
$isiKaryawan = ""; 
	...
	WHERE REVISI=$revisi AND BULAN='$bulan' AND TAHUN='$tahun' AND MMG.TRANS_TUNAI=1";
		//echo $queryGaji;
	...
	while($row = oci_fetch_row($resultGaji))
	{
		...
		if($vTotGajiTransfer > 0){
			...
			$recordDetail = array($vNoRek, $vAtasNama, '', '', '', 'IDR', $vTotGajiTransfer, '', '', 'IBU', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 'extended payment detail1');
...
$recordHeader = array('P', $periode, '', $countData, $sumGaji);
