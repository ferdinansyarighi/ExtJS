...
$sumGaji = 0;
	...
	, K.KEKURANGAN KEKURANGAN
	, (SUM(TRL.LEMBUR) + SUM(SUM_VOLUM * TRL.NOMINAL) + K.KEKURANGAN) TOTAL 
	...
	$queryfilter
	GROUP BY PPF.FULL_NAME, TRL.NOMINAL, PPF.PERSON_ID, K.KEKURANGAN, PPF.PREVIOUS_LAST_NAME";
	//echo $queryGaji;
	...
	while($rowExcel = oci_fetch_row($resultExcel))
	{
		...
		$vTotal = $rowExcel[6];
		$vNorek = $rowExcel[7];
		$vAtasNama = $rowExcel[8];
		$sumGaji = $sumGaji + $vTotal;
		
		$recordDetail = array($vNorek, $vAtasNama, '', '', '', 'IDR', $vTotal, '', '', 'IBU', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 'extended payment detail1');
