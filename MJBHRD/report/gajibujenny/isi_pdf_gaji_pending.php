		...
		if($vNew=='YES'){
			...
			if($vNew=='YES'){
				...
				if($vHariMasuk>0){
					...
					$vTotGajiPokokTRP = $vTotGajiPokok;
				}
			} else {
				$queryJumHari = "SELECT ((SELECT COUNT(*) FROM 
				(SELECT LEVEL AS DNUM FROM DUAL
...
//echo date('H:i:s') , " Add some data" , EOL;
		...
		$pdf->Cell(120, 33, 'Revisi Gaji Bulan Lalu : ', 1, 0, 'L', 0);
		$pdf->Cell(50, 33, number_format($vRevisiGaji, 2, ',', '.'), 1, 0, 'R', 0);
		$pdf->Ln(8.5);	
		...
		$pdf->Ln(2);	
		$pdf->Cell(150, 7.5, '', 0, 0, 'C', 0);	
		$pdf->Cell(120, 7.5, 'Pending gaji : ', 0, 0, 'L', 0);
		$pdf->Cell(50, 7.5, number_format($vPending, 2, ',', '.'), 0, 0, 'R', 0);	
		$pdf->Ln(3);
