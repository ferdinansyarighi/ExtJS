...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('AH3', 'Pending Gaji')
            ->setCellValue('AI3', 'Total diTransfer')
            ->setCellValue('AJ3', 'Shift')
            ->setCellValue('AK3', 'Tgl Input Shift');
...
$countHeader = 0;
	...
	--            order by peef.assignment_id)rg
            , ( SELECT  X.PERSON_ID, NVL( SUM( X.OUTSTANDING_P ), 0 ) OUTSTANDING_P, 0 OUTSTANDING_B,
                        NVL( SUM( X.HARGA_CICILAN_P ), 0 ) HARGA_CICILAN_P, 0 HARGA_CICILAN_B
                FROM    (
                    SELECT  PJNM_HDR.ID, PJNM_HDR.PERSON_ID, PJNM_HDR.HARGA_CICILAN_P, PJNM_HDR.JUMLAH_CICILAN AS JUMLAH_CICILAN_P,
                            PJNM_HDR.TOTAL_PINJAMAN, PNJM_DTL.TOTAL_DETAIL,
                            ( NVL( PJNM_HDR.TOTAL_PINJAMAN, 0 ) - NVL( PNJM_DTL.TOTAL_DETAIL, 0 ) ) AS OUTSTANDING_P
                    FROM 
                    (
                        (
                        SELECT  MTP.ID, MTP.PERSON_ID, MTP.NOMINAL HARGA_CICILAN_P, MTP.JUMLAH_CICILAN,
                                ( NVL( MTP.NOMINAL, 0 ) * NVL( MTP.JUMLAH_CICILAN, 0 ) ) AS TOTAL_PINJAMAN
                    ...
                    (
                        SELECT  MJ_T_PINJAMAN_ID, NVL( SUM( NOMINAL ), 0 ) TOTAL_DETAIL
                        FROM    MJ_T_PINJAMAN_DETAIL
                        GROUP   BY MJ_T_PINJAMAN_ID
                    ) PNJM_DTL
                    WHERE PJNM_HDR.ID = PNJM_DTL.MJ_T_PINJAMAN_ID (+)
                    AND     ( NVL( PJNM_HDR.TOTAL_PINJAMAN, 0 ) - NVL( PNJM_DTL.TOTAL_DETAIL, 0 ) ) <> 0
                ) X
	...
	--and     p.person_id in(13985,37870,37250, 40930)--33617
	...
	while($row = oci_fetch_row($resultGaji))
	{
		...
		if($vResign=='YES' && $revisi == 1){
			...
			$queryJamLembur2 = "SELECT MTSD.TOTAL_JAM, NVL(MTT.JAM_MASUK, '16:00'), NVL(MTT.JAM_KELUAR, NVL(MTSI.JAM_TO, NVL(MTT2.JAM_KELUAR, '23:59'))), MTS.TANGGAL_SPL        
			FROM MJ.MJ_T_SPL MTS
			INNER JOIN MJ.MJ_T_SPL_DETAIL MTSD ON MTSD.MJ_T_SPL_ID=MTS.ID
			LEFT JOIN MJ.MJ_T_TIMECARD MTT ON MTT.TANGGAL=MTS.TANGGAL_SPL AND MTT.STATUS=253 AND MTT.PERSON_ID=$vPersonId
			LEFT JOIN MJ.MJ_T_TIMECARD MTT2 ON MTT2.TANGGAL=MTS.TANGGAL_SPL AND MTT2.STATUS=248 AND MTT2.PERSON_ID=$vPersonId
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('AH' . $xlsRow, $pending_gaji)
					->setCellValue('AI' . $xlsRow, $vTotGajiTransfer)
					->setCellValue('AJ' . $xlsRow, $Hours)
					->setCellValue('AK' . $xlsRow, $tglHours);
	}
