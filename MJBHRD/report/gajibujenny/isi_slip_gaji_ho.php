...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('H3', 'Grade')
            ->setCellValue('I3', 'Lokasi / Plant')
            ->setCellValue('J3', 'Tanggal Masuk')
            ->setCellValue('K3', 'Hari Masuk')
            ->setCellValue('L3', 'Hari Sakit')
            ->setCellValue('M3', 'Hari Cuti')
            ->setCellValue('N3', 'Hari Ijin')
            ->setCellValue('O3', 'Hari Alpha')
            ->setCellValue('P3', 'Terlambat 1')
            ->setCellValue('Q3', 'Terlambat 2')
            ->setCellValue('R3', 'Terlambat 3')
            ->setCellValue('S3', 'Terlambat 4')
            ->setCellValue('T3', 'Gaji Pokok')
            ->setCellValue('U3', 'Tunjangan Jabatan')
            ->setCellValue('V3', 'Tunjangan Lokasi')
            ->setCellValue('W3', 'Tunjangan Grade')
            ->setCellValue('X3', 'Uang Makan')
            ->setCellValue('Y3', 'Uang Transport')
            ->setCellValue('Z3', 'Premi Hadir')
            ->setCellValue('AA3', 'Lembur')
            ->setCellValue('AB3', 'PPH21')
            ->setCellValue('AC3', 'BPJS Kesehatan')
            ->setCellValue('AD3', 'BPJS TK')
            ->setCellValue('AE3', 'Pinjaman')
            ->setCellValue('AF3', 'BS')
            ->setCellValue('AG3', 'Absen')
            ->setCellValue('AH3', 'Telat')
            ->setCellValue('AI3', 'Total Gaji')
            ->setCellValue('AJ3', 'Revisi Gaji Bulan Lalu')
            ->setCellValue('AK3', 'Pending Gaji')
            ->setCellValue('AL3', 'Total diTransfer')
            ->setCellValue('AM3', 'Shift')
            ->setCellValue('AN3', 'Tgl Input Shift')
            ->setCellValue('AO3', 'Kelengkapan Rekening')
			->setCellValue('AP3', 'Nomor Rekening')
            ->setCellValue('AQ3', 'Nama Rekening')
            ->setCellValue('AR3', 'Nama Bank')
            ->setCellValue('AS3', 'Group Gaji')
            ->setCellValue('AT3', 'Payroll');
...
$countHeader = 0;
	...
	, pos.position_id
    , hou.name perusahaan
	, NVL(p.ATTRIBUTE2, 'Tidak')
	, PPG.GROUP_NAME
	, TRIM(REGEXP_SUBSTR(p.PREVIOUS_LAST_NAME, '[^-]+', 1, 1)) BANK_REK
	, TRIM(REGEXP_SUBSTR(p.PREVIOUS_LAST_NAME, '[^-]+', 1, 2)) NO_REK
	, TRIM(REGEXP_SUBSTR(p.PREVIOUS_LAST_NAME, '[^-]+', 1, 3)) NAME_REK
	, PAPF.PAYROLL_NAME
	, pg.NAME GRADE_NAME
	...
	--and     p.person_id = 43952--in(13985,37870,37250, 40930)--33617
	...
	while($row = oci_fetch_row($resultGaji))
	{
		...
		$vNomorRekening = $row[28];
		$vNamaRekening = $row[29];
		$vNamaBank = $row[27];
		$vGroupGaji = $row[26];
		$vPayroll = $row[30];
		$vGradeName = $row[31];
		...
		while($rowGaji = oci_fetch_row($resultGaji)){
			...
			switch($element){
				...
				case 'E_UANG_MAKAN':
					...
					break;
				case 'E_UANG_TRANSPORT':
					$uangTransport = $value * $vJumMasuk;
		...
		$vTD = ((($vGajiPokok + $vTunJab + $vTunGrade + $vTunLok + $uangMakan + $uangTransport) * 0.2) / 25);
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('H' . $xlsRow, $vGradeName)
					->setCellValue('I' . $xlsRow, $vLokasi)
					->setCellValue('J' . $xlsRow, $hireDate)
					...
					->setCellValue('T' . $xlsRow, $vTotGajiPokok)
					->setCellValue('U' . $xlsRow, $vTunJab)
					->setCellValue('V' . $xlsRow, $vTunLok)
					->setCellValue('W' . $xlsRow, $vTunGrade)
					->setCellValue('X' . $xlsRow, $vTotUangMakan)
					->setCellValue('Y' . $xlsRow, $vTotUangTrans)
					->setCellValue('Z' . $xlsRow, $vTotTunKer)
					->setCellValue('AA' . $xlsRow, $vTotUangLembur)
					->setCellValue('AB' . $xlsRow, 0)
					->setCellValue('AC' . $xlsRow, $vBPJSKS)
					->setCellValue('AD' . $xlsRow, $vBPJSTK)
					->setCellValue('AE' . $xlsRow, $vPotongan)
					->setCellValue('AF' . $xlsRow, $vBS)
					->setCellValue('AG' . $xlsRow, $vTotUangAbsen)
					->setCellValue('AH' . $xlsRow, $vTotUangTerlambat)
					->setCellValue('AI' . $xlsRow, $vTotGajiSkr)
					->setCellValue('AJ' . $xlsRow, $vRevisiGaji)
					->setCellValue('AK' . $xlsRow, $pending_gaji)
					->setCellValue('AL' . $xlsRow, $vTotGajiTransfer)
					->setCellValue('AM' . $xlsRow, $Hours)
					->setCellValue('AN' . $xlsRow, $tglHours)
					->setCellValue('AO' . $xlsRow, $vKelengkapan)
					// add RRU 20190408
					->setCellValueExplicit('AP' . $xlsRow, $vNomorRekening, PHPExcel_Cell_DataType::TYPE_STRING)
					->setCellValue('AQ' . $xlsRow, $vNamaRekening)
					->setCellValue('AR' . $xlsRow, $vNamaBank)
					->setCellValue('AS' . $xlsRow, $vGroupGaji)
					->setCellValue('AT' . $xlsRow, $vPayroll);
	}
